#!/usr/bin/env python3
"""
Experiment 67: Llama-3.2-3B port to Blackhole.

Scaling test: 3x parameters vs Llama-3.2-1B (3.2B vs 1.24B).
Architecture: 28 layers, 3072 hidden, 24Q/8KV heads, head_dim=128, intermediate=8192.
Same split SDPA workaround (8 KV → 2×4). Same interleaved RoPE.
Multi-shard safetensor loading. Tied embeddings.

Hypothesis: ~30-40 tok/sec (2.5-3x slower than 1B due to wider matmuls).
"""
...
model_ids = ["meta-llama/Llama-3.2-3B", "unsloth/Llama-3.2-3B"]
...
print(f"  embed_w: {embed_w.shape}, hidden: {embed_w.shape[1]}")
...
# Rotation matrix for interleaved format
...
def decode_forward():
    ...
    for i in range(n_layers):
        ...
        # Split SDPA: 2 groups of (12Q, 4KV)
...
print(f"  Text: {text}")
print("\n  Comparison:")
print("    Qwen2.5-0.5B:  7.1ms/tok (140 tok/sec)")
print("    Qwen3-0.6B:   13.2ms/tok  (76 tok/sec)")
print("    Llama-3.2-1B:  12.8ms/tok  (78 tok/sec)")
print(f"    Llama-3.2-3B:  {avg_ms:.1f}ms/tok ({tps:.0f} tok/sec)")
