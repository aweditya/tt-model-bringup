#!/usr/bin/env python3
"""
Experiment 57c: bf8 MLP weights with traced KV-cached decode.

57b showed all configs work at 20 tokens (full recompute).
This tests the best mixed config (bf8 MLP + bf16 attention) with
our production traced decode path over 100 tokens.

Hypothesis: bf8 MLP weights save memory and give ~10% speed boost
without affecting generation quality, since MLP matmuls showed 0.9999
per-op cosine and the attention path (precision-sensitive) stays bf16.
"""
...
def decode_forward():
    ...
    for i in range(n_layers):
        ...
        q = ttnn.reshape(ttnn.add(ttnn.matmul(h, dl["q_w"], compute_kernel_config=hifi4), dl["q_b"]), [1,n_q_heads,1,head_dim])
        k = ttnn.reshape(ttnn.add(ttnn.matmul(h, dl["k_w"], compute_kernel_config=hifi4), dl["k_b"]), [1,n_kv_heads,1,head_dim])
        v = ttnn.reshape(ttnn.add(ttnn.matmul(h, dl["v_w"], compute_kernel_config=hifi4), dl["v_b"]), [1,n_kv_heads,1,head_dim])
...
print(f"\n{'='*60}")
print(f"RESULTS: bf8 MLP + bf16 attention (traced decode)")
print(f"{'='*60}")
print(f"  Trace exec: {avg:.1f}ms/tok ({1000/avg:.1f} tok/sec)")
print(f"  Tokens generated: {len(trace_times)}")
print(f"  Text: {text}")
print(f"\n  Comparison:")
print(f"    53e all-bf16:  7.6ms/tok (132 tok/sec)")
print(f"    57c bf8-MLP:   {avg:.1f}ms/tok ({1000/avg:.1f} tok/sec)")
