#!/usr/bin/env python3
"""
Experiment 90: Qwen1.5-MoE-A2.7B — Full 24-Layer Eager Decode

First MoE text generation on Tenstorrent Blackhole.
  - 14.3B params, 2.7B active per token
  - 24 layers, 60 experts (top-4), MHA (16Q/16KV), head_dim=128
  - Eager decode with CPU routing, device expert execution
  - ~5 tok/s expected (dispatch-bound); tracing comes in exp 91

Run: ssh tenstorrent 'cd tt-xla && python3 experiments/90_moe_full_model.py'
"""
...
# ── Device ───────────────────────────────────────────────────
print("=" * 60)
print("Exp 90: Qwen1.5-MoE-A2.7B — Full Model on Blackhole")
print("=" * 60)
...
def prefill(token_ids):
    ...
    for i in range(n_layers):
        ...
        if i == 0:
            print(f"    Layer 0 input norm: {np.linalg.norm(x_np):.4f}")
        ...
        if dl["seg_w"] is not None:
            # Shared expert gate: linear [2048, 1] + sigmoid — per-token gating
            seg_logit = h2_np @ from_dev(dl["seg_w"], (hidden, 1))  # Use numpy for [B*T, 1]
        ...
        if i < 3 or (i + 1) % 6 == 0:
            ...
            top_e = list(sorted(active)[:5])
            print(f"    Layer {i+1}/{n_layers}: norm={norm:.2f}, active_experts={len(active)}, sample={top_e}")
    ...
    print(f"    Top-5 predictions: {[(int(t), tokenizer.decode([t]), float(last[t])) for t in top5]}")
    return last


# ── Eager decode step ────────────────────────────────────────
def decode_step(token_id, pos):
    """Single-token decode: device attention + top-4 MoE + CPU routing."""
    ...
    for i in range(n_layers):
        ...
        # MoE: router on device → CPU top-4 → device experts
        ...
        # Run top-4 experts, accumulate on CPU
        moe_np = np.zeros((1, hidden), dtype=np.float32)
        for e in top4:
            ew = dl["experts"][e]
            ...
            d = ttnn.matmul(ttnn.mul(ttnn.silu(g), u), ew["d"], compute_kernel_config=hifi4)
            ttnn.synchronize_device(device)
            moe_np += probs[e] * from_dev(d, (1, hidden))
        ...
        sd = ttnn.matmul(ttnn.mul(ttnn.silu(sg), su), dl["s_down_w"], compute_kernel_config=hifi4)
        ttnn.synchronize_device(device)
        sd_np = from_dev(sd, (1, hidden))
        if dl["seg_w"] is not None:
            h2_np_decode = from_dev(h2, (1, hidden))
            seg_logit = h2_np_decode @ from_dev(dl["seg_w"], (hidden, 1))
            seg_val = 1.0 / (1.0 + np.exp(-seg_logit))
            moe_np += seg_val * sd_np
        else:
            moe_np += sd_np

        # Residual: read x, add moe on CPU, re-upload
        x_np = from_dev(x, (1, 1, hidden))
        x_np = x_np.reshape(1, hidden) + moe_np
        x = to_bf16(x_np.reshape(1, 1, hidden))
...
for prompt in prompts:
    ...
    # Decode
    pos = len(tokens)
    eos_ids = {tokenizer.eos_token_id, 151643, 151644, 151645}  # im_end, endoftext, im_start
    max_gen = 100
    times = []
    ...
    print(f"\n  Output: {text}")
    print(f"  {len(gen)} tokens | {avg_ms:.0f} ms/tok | {tok_s:.1f} tok/s")

# ── Summary ──────────────────────────────────────────────────
print(f"\n{'=' * 60}")
print("SUMMARY")
print(f"{'=' * 60}")
print(f"  Model: Qwen1.5-MoE-A2.7B (14.3B total, 2.7B active)")
print(f"  Mode: Eager decode (top-4 routing, CPU accumulation)")
print(f"  Expert weights: BFP8, Attention: bf16")
print(f"  Next: Exp 91 — traced decode (all 60 experts, ~30 tok/s target)")
