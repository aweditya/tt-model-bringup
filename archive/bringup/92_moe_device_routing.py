#!/usr/bin/env python3
"""
Experiment 92: Qwen1.5-MoE-A2.7B-Chat — Device-Side MoE Routing

Key improvements vs exp 91 (20.2 tok/s):
  1. On-device softmax (ttnn.softmax) — no CPU softmax
  2. On-device top-4 (ttnn.topk) — no CPU argsort
  3. On-device shared expert gate (ttnn.sigmoid) — no h2 readback
  4. Only 1 sync per layer: read 8 floats (4 indices + 4 probs) = 32 bytes
     (vs exp 91: 60 floats router logits + 2048 floats h2 = ~8.5 KB)

The decode step is now almost entirely on-device. The only CPU intervention
is reading top-4 expert indices to select which weight tensors to use.

Target: ~22-25 tok/s (eliminating ~1-2ms of readback overhead per token)

Run: ssh tenstorrent 'cd tt-xla && python3 experiments/92_moe_device_routing.py'
"""
...
# ── Device ───────────────────────────────────────────────────
print("=" * 60)
print("Exp 92: Qwen1.5-MoE-A2.7B-Chat — Device-Side Routing")
print("=" * 60)
...
def decode_step(token_id, pos):
    ...
    for i in range(n_layers):
        ...
        # ── Shared expert: fully on device including gate ────
...
# ── Summary ──────────────────────────────────────────────────
print(f"\n{'=' * 60}")
print("SUMMARY — Experiment 92: Device-Side MoE Routing")
print(f"{'=' * 60}")
print(f"  Model: Qwen1.5-MoE-A2.7B-Chat (14.3B total, 2.7B active)")
print(f"  New in exp 92:")
print(f"    - ttnn.softmax for routing (no CPU softmax)")
print(f"    - ttnn.topk(k=4) for expert selection (no CPU argsort)")
print(f"    - ttnn.sigmoid for shared expert gate (no h2 readback)")
print(f"    - Only 32 bytes read per layer (vs ~8.5 KB in exp 91)")
print(f"  Baseline exp 91: 20.2 tok/s")
