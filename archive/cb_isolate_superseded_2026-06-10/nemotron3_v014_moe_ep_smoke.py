#!/usr/bin/env python3
"""MM7 v0.1.4 — L1 MoE Expert-Parallel forward smoke.

Gates the True-EP forward (`moe_block_eager_ep`) against the HF oracle.
Same three gates as v0.1.3.b (so the refactor is provably bit-equivalent
to the naive replicated version that already PASSES):

  S — shared_out  cos vs HF L1_moe_shared_out
  M — mixer_out   cos vs HF L1_moe_mixer_out
  B — block_out   cos vs HF hidden_states[2]

REUSE: forks v0.1.3.b smoke; only the forward function called differs.

Run on the QuietBox (default: EP mode):
    cd ~/tt-xla && NEMOTRON3_UPLOAD_LAYERS=1 \\
        ...
        .venv/bin/python -u experiments/cb/isolate/nemotron3_v014_moe_ep_smoke.py
"""
...
def main() -> int:
    import os
    os.environ.setdefault("NEMOTRON3_UPLOAD_LAYERS", str(L1))
    ...
    log("bootstrapping (L1 — Expert Parallel: 32 experts/chip)…")
    ...
    try:
        ...
        log("running TT MoE block EP path "
            "(pre-norm + router + all_to_all_dispatch + local experts + "
            "all_to_all_combine + shared + residual)…")
        t_fwd = time.time()
        res = srv.moe_block_eager_ep(state, h_input, L1)
        log(f"  forward in {time.time() - t_fwd:.1f}s")
        for k in ["shared_out", "mixer_out", "block_out"]:
            arr = res[k]
            log(f"  {k} shape: {arr.shape}  range=[{arr.min():.3f}, {arr.max():.3f}]")
