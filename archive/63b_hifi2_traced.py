#!/usr/bin/env python3
"""
Experiment 63b: HiFi2 MLP + HiFi4 attention in traced decode.

From 63: HiFi2 MLP = 29% faster in full recompute (1190ms vs 1674ms).
Config mixing works when SDPA stays HiFi4 (no kernel config leak).

Test in traced decode:
  - Current best: 7.1ms/tok (bf8 MLP + native RoPE + HiFi4 everywhere)
  - If HiFi2 MLP saves 29%, could get ~5.0ms/tok = 200 tok/sec (!!)
  - More realistically: trace overhead dominates, expect 10-15% improvement
"""
...
# Upload: bf8 weights + bf16 biases/norms
print("Uploading weights (bf8 all weights)...")
...
# Traced decode: HiFi4 attention + HiFi2 MLP + native RoPE
def decode_forward():
    x = embed_buf
    for i in range(n_layers):
        dl = dev_layers[i]
        h = ttnn.rms_norm(x, weight=dl["ln1_g"], epsilon=rms_eps)
        # Attention path: HiFi4
...
print(f"\n{'='*60}")
print(f"RESULTS: bf8 all + HiFi2 MLP + HiFi4 attn + native RoPE")
print(f"{'='*60}")
print(f"  Trace exec: {avg:.1f}ms/tok ({1000/avg:.1f} tok/sec)")
print(f"  Tokens: {len(trace_times)}")
print(f"  Text: {text}")
print(f"\n  Comparison:")
print(f"    53e  bf16+HiFi4+rotmat:      7.6ms (132 tok/sec)")
print(f"    60   bf8MLP+HiFi4+native:    7.1ms (140 tok/sec)")
print(f"    62   bf8ALL+HiFi4+native:    7.1ms (140 tok/sec)")
print(f"    63b  bf8ALL+HiFi2MLP+native: {avg:.1f}ms ({1000/avg:.0f} tok/sec)")
