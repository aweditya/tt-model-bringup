#!/usr/bin/env python3
"""
Experiment 43: Qwen2.5-0.5B correctness validation.

Computes a pure-numpy/torch reference forward pass (no transformers dependency)
and compares per-layer against the TT-NN implementation.

This avoids the torchvision import crash entirely.
"""
...
print("=" * 60)
print("Experiment 43: Qwen2.5-0.5B Correctness Validation")
print("Pure numpy reference vs TT-NN — per-layer comparison")
print("=" * 60)
...
# Keep a copy for numpy reference (before we delete)
...
def apply_rope_np(x_4d, n_heads):
    """Apply RoPE. x: (B, n_heads, T, head_dim)."""
    t = x_4d.shape[2]
    x_even = x_4d[..., 0::2]
    x_odd = x_4d[..., 1::2]
...
# ═══════════════════════════════════════════════════════════
# NUMPY REFERENCE FORWARD (float32, no TT-NN)
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
...
ref_layer_outputs = [ref_x.copy()]  # hidden_0 = embedding output
ref_attn_outputs = []
ref_mlp_outputs = []
...
for i in range(n_layers):
    ...
    # GQA: expand KV heads to match Q heads (7:1 ratio)
    kv_repeat = n_q_heads // n_kv_heads  # 7
    k_4d = np.repeat(k_4d, kv_repeat, axis=1)  # (B, 14, T, 64)
    v_4d = np.repeat(v_4d, kv_repeat, axis=1)  # (B, 14, T, 64)

    # Attention (float32 softmax)
    scale = 1.0 / np.sqrt(head_dim)
    scores = (q_4d @ k_4d.transpose(0, 1, 3, 2)) * scale  # (B, 14, T, T)
    ...
    attn_out = attn_weights @ v_4d  # (B, 14, T, 64)
...
# ═══════════════════════════════════════════════════════════
# TT-NN FORWARD (per-layer comparison)
# ═══════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("Phase 2: TT-NN forward with per-layer comparison")
print("=" * 60)
