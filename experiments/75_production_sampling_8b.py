#!/usr/bin/env python3
"""
Experiment 75: Production sampling on Llama-3.1-8B-Instruct.

Exp 72 showed sampling doesn't fix 3B models — they lack capacity.
Exp 73 showed 8B with greedy degenerates at ~50 tokens.
Research (research/long_form_generation.md) says the fix is:
  temp=0.7 + min_p=0.05 + rep_penalty=1.1

This is the real test: can 8B + production sampling sustain 200+ coherent tokens?

Hypothesis: 8B-Instruct with proper sampling produces coherent multi-paragraph text.
"""
...
def sample_greedy(logits, generated_ids):
    return int(np.argmax(logits))


def sample_production(logits, generated_ids, temperature=0.7, min_p=0.05, rep_penalty=1.1, rep_window=64):
    """Production sampling: temp + min-p + repetition penalty.

    This is the llama.cpp / Ollama standard as of 2026.
    """
    # Step 1: Repetition penalty (before temperature)
    ...
    # Step 4: Min-p filtering
    top_prob = probs.max()
    threshold = min_p * top_prob
    mask = probs >= threshold
    if mask.sum() == 0:
        mask[np.argmax(probs)] = True  # always keep at least the top token
...
def sample_top_p(logits, generated_ids, temperature=0.7, top_p=0.9, rep_penalty=1.1, rep_window=64):
...
def generate(prompt_tokens, max_tokens, sampler, sampler_name=""):
    reset_kv_caches()

    tt_logits = ttnn_prefill(np.array(prompt_tokens))
    gen_tokens = []
    next_id = sampler(tt_logits.copy(), gen_tokens)
    ...
    for step in range(max_tokens - 1):
        ...
        next_id = sampler(logits.copy(), gen_tokens)
        gen_tokens.append(next_id)
        pos += 1
...
def count_repeated_trigrams(text):
    """Count fraction of repeated 3-grams in generated text."""
...
strategies = [
    ...
    ("Production (temp=0.7, min_p=0.05, rep=1.1)", sample_production),
    ("Top-p (temp=0.7, p=0.9, rep=1.1)", sample_top_p),
]

test_prompts = [
    # Short factual — baseline
    ("What is the capital of France?", 50),
    # Multi-paragraph explanation — the real test
    ("Explain quantum computing in simple terms. Include what qubits are, "
     "how superposition works, and why quantum computers are faster.", 300),
    # Creative — hardest test
    ("Write a short story about a robot that learns to paint. Include dialogue.", 300),
    # Structured list — tests sustained formatting
    ("List the top 5 programming languages in 2025 and explain why each is popular. "
     "Give at least 2 sentences per language.", 400),
]

results = []

for strat_name, sampler_fn in strategies:
    print(f"\n{'#'*70}")
    ...
    for prompt, max_tok in test_prompts:
        ...
        gen, times = generate(tokens, max_tok, sampler_fn, strat_name)
        text = tokenizer.decode(gen, skip_special_tokens=True)
        sustained = times[1:] if len(times) > 1 else times
        avg_ms = np.mean(sustained) * 1000 if sustained else 0
        rep_frac = count_repeated_trigrams(text)

        hit_eos = gen[-1] in stop_ids if gen else False
        ...
        print(f"  Repeated trigrams: {rep_frac:.1%}")
...
print(f"\n{'='*80}")
print(f"SUMMARY: Llama-3.1-8B-Instruct — Sampling Strategy Comparison")
print(f"{'='*80}")
print(f"{'Strategy':<45} {'Prompt':<25} {'Tok':>4} {'ms/t':>6} {'Rep%':>5} {'EOS':>4}")
print(f"{'-'*45} {'-'*25} {'-'*4} {'-'*6} {'-'*5} {'-'*4}")
for r in results:
    print(f"{r['strategy']:<45} {r['prompt'][:25]:<25} {r['tokens']:>4} "
          f"{r['ms_per_tok']:>5.1f} {r['rep_frac']:>4.0%} {'Y' if r['hit_eos'] else 'N':>4}")

print(f"\n{'='*80}")
print(f"KEY QUESTION: Does production sampling (temp=0.7 + min_p=0.05 + rep=1.1)")
print(f"sustain coherent multi-paragraph text from an 8B model on Blackhole?")
print(f"{'='*80}")
