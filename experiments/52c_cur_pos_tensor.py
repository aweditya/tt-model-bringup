#!/usr/bin/env python3
"""
Experiment 52c: Traced decode with cur_pos_tensor.

Key insight: The KV cache update (update_cache_for_token_) needs int position,
but scaled_dot_product_attention_decode supports cur_pos_tensor (device tensor).

Architecture:
  Non-traced part (per step):
    1. Upload embedding to buffer (ttnn.copy)
    2. Upload RoPE cos/sin to buffer (ttnn.copy)
    3. Upload cur_pos_tensor (int32 device tensor)

  Traced part (runs once, replayed):
    1. Q/K/V projection + on-device RoPE
    2. SDPA decode with cur_pos_tensor
    3. Output projection + MLP

  Non-traced part (per step):
    4. Read logits from device
    5. Read K/V from device, update KV cache at correct position

This hybrid approach keeps the heavy compute (24-layer forward) in a trace
while handling position-dependent ops outside.
"""
...
# cur_pos_tensor buffer (int32)
cur_pos_tensor = ttnn.from_torch(torch.tensor([0], dtype=torch.int32), device=device)
...
def update_buffers(token_id, pos):
    ...
    ttnn.copy(ttnn.from_torch(torch.tensor([pos], dtype=torch.int32), device=device), cur_pos_tensor)

def apply_rope_buf(x_tt):
    """RoPE using buffer values."""
...
# ══════════════════════════════════════════════════════════════
# TEST: Does cur_pos_tensor work with our SDPA decode?
# ══════════════════════════════════════════════════════════════
print("\nTesting cur_pos_tensor...")

# Quick test: create a small Q/K/V and test both cur_pos and cur_pos_tensor
test_q = to_dev_4d(np.random.randn(1, 1, n_q_heads, head_dim).astype(np.float32))
test_pos = 3

try:
    # Standard: cur_pos=[3]
    out1 = ttnn.transformer.scaled_dot_product_attention_decode(
        test_q, k_caches[0], v_caches[0], cur_pos=[test_pos])
    print(f"  cur_pos=[{test_pos}]: shape {out1.shape}")

    # Tensor: cur_pos_tensor
    pos_tensor = ttnn.from_torch(torch.tensor([test_pos], dtype=torch.int32), device=device)
    out2 = ttnn.transformer.scaled_dot_product_attention_decode(
        test_q, k_caches[0], v_caches[0], cur_pos_tensor=pos_tensor)
    print(f"  cur_pos_tensor={test_pos}: shape {out2.shape}")

    # Compare
    o1 = from_dev(out1, (1, 1, n_q_heads, head_dim))
    o2 = from_dev(out2, (1, 1, n_q_heads, head_dim))
    cos_sim = np.dot(o1.flatten(), o2.flatten()) / (np.linalg.norm(o1) * np.linalg.norm(o2) + 1e-8)
    print(f"  Cosine between methods: {cos_sim:.6f}")
    print(f"  ✓ cur_pos_tensor works!")
except Exception as e:
    print(f"  ✗ cur_pos_tensor failed: {e}")
    import traceback
    traceback.print_exc()
...
# ══════════════════════════════════════════════════════════════
# Non-traced decode (fully on-device, baseline)
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# RUN: Non-traced decode (correct, baseline speed)
# ══════════════════════════════════════════════════════════════
...
if decode_times:
    ...
    print(f"\nFully on-device (correct, non-traced):")
...
print(f"    Exp 52:   7ms/tok (135.6 tok/sec) — traced but stale positions")
if decode_times:
    print(f"    Exp 52c: {avg:.0f}ms/tok ({1000/avg:.1f} tok/sec) — correct, non-traced")

print(f"\n  cur_pos_tensor: verified working")
print(f"  Next: hybrid traced decode with KV update outside trace")
