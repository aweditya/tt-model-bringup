#!/usr/bin/env python3
"""
Experiment 98: Inter-Layer Pipelining — Overlap Routing Sync with Next Attention

Building on exp 95 (22.7 tok/s, partial trace).

Problem: Each layer has a sync barrier for routing readback (0.28ms per layer = 6.7ms).
The sync blocks CQ0 from starting the next layer's attention.

Insight: After routing dispatch (softmax+topk), we can START the next layer's
attention trace BEFORE reading routing results. The attention doesn't depend
on routing — it depends on the previous layer's output (x2 = x + attn_out).

Pipeline structure:
  Layer 0: [attention] [routing dispatch]
  Layer 1:                                [attention] [routing dispatch]
           [sync L0 routing] [L0 experts]              [sync L1 routing] [L1 experts]

But there's a dependency: layer i+1's attention input = layer i's full output (x2 + moe).
So we can't truly overlap — the experts must finish before next attention starts.

Alternative: overlap routing READBACK with expert execution on same layer.
Use multi-CQ: CQ0 runs experts, CQ1 reads routing results for NEXT layer.

Actually, the real win is: use 2 CQs to overlap the device sync with compute.
  CQ0: [attention_i] [routing_i] [experts_i using routing_i-1]
  CQ1: [read routing_i-1 results]

This way, while CQ0 is running attention+routing for layer i, CQ1 is reading
back routing results from layer i-1 that we'll need for layer i's experts.

But wait — we need routing results BEFORE we can dispatch experts (to know which
experts to call). So the real overlap is:

  For each layer:
    1. Execute attention trace (CQ0, non-blocking)
    2. Execute routing (CQ0): rms_norm + matmul + softmax + topk
    3. Record event on CQ0
    4. CQ1: wait for event, then read back routing results
    5. Meanwhile on CQ0: dispatch shared expert (doesn't need routing results!)
    6. Sync CQ1 to get routing indices
    7. CQ0: dispatch top-4 experts

This overlaps the routing readback with the shared expert computation (~4ms)!
Routing sync = 0.28ms, shared expert = ~4ms → fully hidden!

Savings: 0.28ms × 24 layers = 6.7ms → ~37ms → ~27 tok/s

Run: ssh tenstorrent 'cd tt-xla && python3 experiments/98_moe_pipeline.py'
"""
...
print("=" * 60)
print("Exp 98: Inter-Layer Pipelining")
print("=" * 60)
...
CQ0 = 0  # Compute queue
CQ1 = 1  # I/O queue (for routing readback)
...
def decode_step(token_id, pos):
    """
    Pipelined decode: overlap routing readback with shared expert compute.

    Per layer:
      1. Execute attention trace (CQ0)
      2. Dispatch routing: rms_norm + matmul + softmax + topk (CQ0)
      3. Record event (CQ0) — routing results are on device
      4. Start shared expert computation (CQ0) — doesn't need routing!
      5. Wait for routing event (CQ1), read back routing indices
      6. After shared expert finishes + routing read completes:
         dispatch top-4 experts (CQ0)

    The shared expert takes ~4ms. Routing readback takes ~0.28ms.
    So we fully hide the readback behind shared expert compute.
    """
    ...
    for i in range(n_layers):
        ...
        if MULTI_CQ:
            # ── 3. Record event: routing done ────────────────
            route_event = ttnn.record_event(device, cq_id=CQ0)

            # ── 4. Start shared expert BEFORE reading routing ─
            # Shared expert doesn't depend on which routed experts are selected!
            ...
            # ── 5. Read routing results via CQ1 ──────────────
            ttnn.wait_for_event(CQ1, route_event)
            # Need to sync CQ0 too because shared expert is still running on CQ0
            # and from_dev needs the tensor to be ready
            ttnn.synchronize_device(device)
            ...
            if dl["seg_w"] is not None:
                ...
                shared_out = ttnn.mul(sd, seg_val)
            else:
                shared_out = sd
        else:
            # Fallback: same as exp 95
            ttnn.synchronize_device(device)
            top4_vals_np = from_dev(top4_vals, (top_k,))
            top4_idxs_np = from_dev(top4_idxs, (top_k,)).astype(int)
...
# ── Summary ──────────────────────────────────────────────────
print(f"\n{'=' * 60}")
print("SUMMARY — Experiment 98: Inter-Layer Pipelined MoE Decode")
print(f"{'=' * 60}")
print(f"  Model: Qwen1.5-MoE-A2.7B-Chat (14.3B total, 2.7B active)")
print(f"  New in exp 98:")
print(f"    - 2 command queues: CQ0=compute, CQ1=routing readback")
print(f"    - Shared expert dispatched BEFORE routing readback")
print(f"    - Routing readback (~0.28ms) hidden behind shared expert (~4ms)")
print(f"    - Combined with partial trace from exp 95")
print(f"  Multi-CQ available: {MULTI_CQ}")
print(f"  Baseline exp 95: 22.7 tok/s (44ms/tok)")
print(f"  Expected: ~24 tok/s (~42ms/tok) — hiding 6.7ms routing sync")
