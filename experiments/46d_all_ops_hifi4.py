#!/usr/bin/env python3
"""
Experiment 46d: Apply HiFi4+fp32 to ALL ops, not just SDPA.

Hypothesis: Setting compute_kernel_config on SDPA leaks state to subsequent ops.
Fix: Apply the SAME config to matmuls too, so there's no config mismatch.

Also test: what if we do a "reset" by running a dummy matmul with default config
after each SDPA call?
"""
...
print("=" * 60)
print("Experiment 46d: All-ops HiFi4+fp32 (first 6 layers)")
print("=" * 60)
...
hifi4_fp32 = ttnn.WormholeComputeKernelConfig(
...
def run_forward(sdpa_cfg, matmul_cfg):
    """Run forward with specified configs for SDPA and matmul."""
    ...
    for i in range(n_layers):
        ...
        mm_kw = {"compute_kernel_config": matmul_cfg} if matmul_cfg else {}
        q_tt = ttnn.add(ttnn.matmul(h_tt, dl["q_w"], **mm_kw), dl["q_b"])
        k_tt = ttnn.add(ttnn.matmul(h_tt, dl["k_w"], **mm_kw), dl["k_b"])
        v_tt = ttnn.add(ttnn.matmul(h_tt, dl["v_w"], **mm_kw), dl["v_b"])
...
tests = [
    ...
    ("HiFi4+fp32 SDPA / default mm", hifi4_fp32, None),
    ("HiFi4+fp32 ALL ops", hifi4_fp32, hifi4_fp32),
    ("default SDPA / HiFi4+fp32 mm", None, hifi4_fp32),
]
...
print()
print("-" * (36 + 10 * n_layers))

for name, sdpa_cfg, mm_cfg in tests:
    try:
        cosines = run_forward(sdpa_cfg, mm_cfg)
        print(f"{name:<36}", end="")
        for c in cosines:
        ...
        print(f"{name:<36} FAILED: {str(e)[:60]}")
