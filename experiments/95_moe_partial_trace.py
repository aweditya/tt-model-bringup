#!/usr/bin/env python3
"""
Experiment 95: MoE Partial Trace — Trace Attention, Eager MoE

The profiler (exp 94) showed attention = 11.4ms (24% of 48.3ms/tok).
Attention is a static computation graph — no data-dependent branching.
If we trace the attention block, we eliminate ~360 dispatches = ~10ms.

Strategy:
  1. Allocate persistent input/output buffers for each layer's attention
  2. Capture 24 attention traces (one per layer)
  3. During decode: copy input → execute trace → copy output → run MoE eager

Expected improvement: 48ms → ~38ms = ~26 tok/s (vs 21 tok/s)

Run: ssh tenstorrent 'cd tt-xla && python3 experiments/95_moe_partial_trace.py'
"""
...
# ── Device ───────────────────────────────────────────────────
print("=" * 60)
print("Exp 95: MoE Partial Trace — Trace Attention, Eager MoE")
print("=" * 60)
...
# ── Prefill (same as exp 91, not traced) ─────────────────────
...
# For trace capture, we need persistent input buffers.
# The attention block takes: x [1,1,1,hidden], cos [1,1,1,head_dim], sin [1,1,1,head_dim], pos [1]
# And returns: x2 [1,1,1,hidden] = x + attention_output
...
# First: run attention once per layer to warm up program cache
...
# Now capture attention traces per layer
# Each layer needs its own input buffer (x_in) and produces output (x_out)
attn_traces = []
attn_x_ins = []   # persistent input buffers
attn_x_outs = []  # persistent output buffers
...
for i in range(n_layers):
    ...
    # Create a fresh input buffer for this layer (4D to match decode x shape)
...
# ── Decode step with partial trace ───────────────────────────
def decode_step(token_id, pos):
    """Decode with traced attention + eager MoE routing."""
    ...
    for i in range(n_layers):
        ...
        # ── Traced attention ─────────────────────────────────
        ttnn.copy(x, attn_x_ins[i])  # copy input into trace's input buffer
        ttnn.execute_trace(device, attn_traces[i], cq_id=0, blocking=False)
        x2 = attn_x_outs[i]  # trace wrote output here

        # ── MoE: eager (device-side routing + top-4 experts) ──
        h2 = ttnn.rms_norm(x2, weight=dl["ln2_g"], epsilon=rms_eps)
...
# ── Summary ──────────────────────────────────────────────────
print(f"\n{'=' * 60}")
print("SUMMARY — Experiment 95: Partial Trace MoE Decode")
print(f"{'=' * 60}")
print(f"  Model: Qwen1.5-MoE-A2.7B-Chat (14.3B total, 2.7B active)")
print(f"  New in exp 95:")
print(f"    - 24 attention traces captured (one per layer)")
print(f"    - Attention runs via trace replay (no dispatch overhead)")
print(f"    - MoE routing + expert dispatch still eager")
print(f"  Baseline exp 93: 21.1 tok/s (47ms/tok)")
print(f"  Expected: ~26 tok/s (~38ms/tok) — saving ~10ms from attention")
