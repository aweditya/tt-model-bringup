#!/usr/bin/env python3
"""
Experiment 79: Careful sampling — preserve EOS, penalize only content tokens.

Problem diagnosed:
- Greedy: enters repetitive loops (exp 78b)
- Production sampling: suppresses EOS, produces word salad (exp 75)

Hypothesis: If we sample with temperature but PROTECT special tokens
(EOS, BOS, header tokens) from repetition penalty and min-p pruning,
the model can generate longer text while still stopping naturally.

Key changes vs exp 75:
1. Don't apply repetition penalty to special tokens (128000-128009)
2. Ensure EOS token is never pruned by min_p
3. Use milder temperature (0.6 vs 0.7)
"""
...
stop_ids = {eot, 128001}
# All special tokens that should NOT be penalized
SPECIAL_TOKENS = set(range(128000, 128257))  # All Llama 3.1 special tokens
...
# ══════════════════════════════════════════════════════════════
# CAREFUL SAMPLING — protect EOS from repetition penalty
# ══════════════════════════════════════════════════════════════

def sample_careful(logits, generated_ids, temperature=0.6, min_p=0.05, rep_penalty=1.1, rep_window=64):
    """Sampling that protects special tokens from repetition penalty."""
    logits = logits.copy()

    # Step 1: Repetition penalty — SKIP special tokens
    recent = generated_ids[-rep_window:] if generated_ids else []
    for tok_id in set(recent):
        if tok_id in SPECIAL_TOKENS:
            continue  # Don't penalize EOS, BOS, header tokens
        if logits[tok_id] > 0:
    ...
    # Step 4: Min-p filtering — PROTECT special tokens
    top_prob = probs.max()
    threshold = min_p * top_prob
    mask = probs >= threshold
    # Always keep special tokens (especially EOS) as candidates
...
def generate(prompt, max_tokens=300, sampler=None):
    reset_kv_caches()
    tokens = make_chat_tokens(prompt)
    logits = ttnn_prefill(np.array(tokens))

    if sampler:
        next_id = sampler(logits.copy(), [])
    else:
    ...
    for step in range(max_tokens - 1):
        ...
        if sampler:
            next_id = sampler(logits.copy(), gen)
        else:
            next_id = int(np.argmax(logits))
...
prompts = [
    ("Short Q&A", "What is the capital of France?"),
    ("Creative w/ dialogue", "Write a short story about a robot that learns to paint. Include dialogue."),
    ("3-paragraph factual", "Explain quantum computing in exactly 3 paragraphs. Paragraph 1: what it is. Paragraph 2: how qubits work. Paragraph 3: why it matters."),
    ("5-item list", "List exactly 5 benefits of exercise. For each benefit, write exactly 2 sentences explaining it."),
    ("Blog post", "You are writing a technical blog post. Write about how neural networks learn, covering: (1) forward propagation, (2) loss calculation, (3) backpropagation, and (4) gradient descent. Use clear examples."),
]

for name, prompt in prompts:
    np.random.seed(42)
    print(f"\n{'='*70}")
    print(f"[{name}] — Careful Sampling (temp=0.6, min_p=0.05, rep=1.1, EOS protected)")
    print(f"Prompt: {prompt[:70]}{'...' if len(prompt)>70 else ''}")
    print(f"{'='*70}")

    gen = generate(prompt, max_tokens=400, sampler=sample_careful)
