#!/usr/bin/env python3
"""ARCHIVED 2026-05-25 — 12-variant batched-expert-matmul search.

Production landed on variant I (rank-3 sharded W + on-device ttnn.concat to
broadcast h across the expert dim). See moe_forward_ttnn_pattern_a_batched in
experiments/serve/server_35b_ttnn.py and the slim regression test at
experiments/utils/test_batched_expert_matmul_isolated.py.

This file is kept as a log of dead ends — variants A/B/C/L/M/N failed for
different ttnn reasons (rank-4 broadcast, expert-sharded matmul, view-decay
under FFN-chain memory pressure, etc.). Do not run it from CI; it is
documentation.
