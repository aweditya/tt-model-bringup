...
void Nemotron3Mamba2DecodeOwnedDeviceOperation::validate_on_program_cache_miss(
    ...
    TT_FATAL(ssm_state.dtype() == DataType::FLOAT32,
             "ssm_state must be fp32 (decision D4 — bf16 recurrent state drifts)");
    TT_FATAL(B_in.padded_shape() == C_in.padded_shape(),
             "B and C must have the same shape");
    TT_FATAL(x.padded_shape() == z.padded_shape(),
...
Nemotron3Mamba2DecodeOwnedDeviceOperation::compute_output_specs(
    ...
    TensorSpec y_spec(
        ...
        TensorLayout(
            x.dtype(),
            PageConfig(Layout::TILE),
            args.output_memory_config.value_or(x.memory_config())));
...
Nemotron3Mamba2DecodeOwnedDeviceOperation::create_output_tensors(
    ...
    Tensor y_out = tensor_args.preallocated_y.has_value()
                       ...
                       : tt::tt_metal::create_device_tensor(
                             std::get<1>(specs), tensor_args.x.device());
