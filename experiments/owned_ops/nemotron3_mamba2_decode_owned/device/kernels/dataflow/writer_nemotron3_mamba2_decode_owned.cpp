...
void kernel_main() {
    ...
    for (uint32_t block = 0; block < num_blocks; ++block) {
        ...
        write_n_tiles_from_cb(state_tensor, global_block * state_tiles_per_block,
                              state_tiles_per_block, cb_state_out);

        // Drain y (head_dim_tiles bf16 tiles).
        write_n_tiles_from_cb(y_tensor, global_block * head_dim_tiles,
                              head_dim_tiles, cb_y);
    }
}
