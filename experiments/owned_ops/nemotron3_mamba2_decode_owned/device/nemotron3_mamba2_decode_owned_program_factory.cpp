...
Nemotron3Mamba2DecodeOwnedProgramFactory::cached_program_t Nemotron3Mamba2DecodeOwnedProgramFactory::create(
    ...
    create_circular_buffer(program, all_cores, CB_STATE_IN,
    ...
    create_circular_buffer(program, all_cores, CB_STATE_SCALED,
                           head_dim_tiles * ssm_state_tiles * 2, fp32_tile_size, fp32_format);
...
void Nemotron3Mamba2DecodeOwnedProgramFactory::override_runtime_arguments(
    ...
    std::vector<std::vector<uint32_t>> reader_runtime_args(
        shared.cores.size(), std::vector<uint32_t>(13, 0));
    ...
    std::vector<std::vector<uint32_t>> compute_runtime_args(
        shared.cores.size(), std::vector<uint32_t>(4, 0));
    ...
    std::vector<std::vector<uint32_t>> writer_runtime_args(
        shared.cores.size(), std::vector<uint32_t>(6, 0));

    uint32_t blocks_written = 0;
    for (uint32_t i = 0; i < shared.num_cores; ++i) {
        const uint32_t blocks_per_core =
            i < shared.g1_numcores ? shared.num_blocks_per_core_group_1
                                   : shared.num_blocks_per_core_group_2;
