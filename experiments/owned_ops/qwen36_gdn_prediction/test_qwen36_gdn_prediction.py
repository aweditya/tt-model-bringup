#!/usr/bin/env python3
"""Device correctness gate for the owned qwen36_gdn_prediction op.
...
def _prediction_rows(state_scaled: np.ndarray, k: np.ndarray) -> np.ndarray:
    pred = oracle.prediction(state_scaled, k)
    return np.broadcast_to(pred[:, None, :], (pred.shape[0], 32, pred.shape[1])).astype(np.float32, copy=True)


def _native_expected(expected: np.ndarray, state_scaled: np.ndarray, k: np.ndarray, args: argparse.Namespace) -> np.ndarray:
    ...
    k_bf16 = _to_bfloat16_numpy(k)
    return _to_bfloat16_numpy(_prediction_rows(state_bf16, k_bf16))


def _progress(message: str) -> None:
    print(f"[qwen36_gdn_prediction] {message}", file=sys.stderr, flush=True)
...
def main() -> int:
    ...
    if not hasattr(ttnn.experimental, "qwen36_gdn_prediction"):
        raise RuntimeError("ttnn.experimental.qwen36_gdn_prediction is not registered in this TTNN build")

    fixture = _load_or_make_fixture(args)
    state_scaled = oracle.decay_state(fixture["state"], fixture["alpha"])
    state_host = np.ascontiguousarray(state_scaled[None, :, :, :], dtype=np.float32)
    k_host = np.broadcast_to(
        fixture["k"][None, :, None, :],
        (1, fixture["k"].shape[0], 32, fixture["k"].shape[1]),
    ).astype(np.float32, copy=True)
    if args.debug_fill:
        fp32_expected = np.ones((fixture["state"].shape[0], 32, fixture["state"].shape[2]), dtype=np.float32)
    else:
        fp32_expected = _prediction_rows(state_scaled, fixture["k"])
    expected = _native_expected(fp32_expected, state_scaled, fixture["k"], args)
    ...
    try:
        ...
        out_tt = ttnn.experimental.qwen36_gdn_prediction(
            state_tt,
            k_tt,
    ...
    payload: dict[str, Any] = {
        "op": "ttnn.experimental.qwen36_gdn_prediction",
        "config": {
        ...
        "upload_k": upload_k_report,
        "actual_stats": _array_stats(actual),
        "expected_stats": _array_stats(expected),
        "input_state_scaled_stats": _array_stats(state_scaled),
