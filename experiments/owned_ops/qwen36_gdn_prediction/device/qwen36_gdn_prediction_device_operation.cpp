...
#include "qwen36_gdn_prediction_device_operation.hpp"
...
void Qwen36GdnPredictionDeviceOperation::validate_on_program_cache_miss(
    ...
    validate_tiled_device_tensor(tensor_args.k, state_scaled, "k");
    TT_FATAL(tensor_args.k.dtype() == state_scaled.dtype(), "k dtype must match state_scaled dtype");
    ...
    const auto& k_logical = tensor_args.k.logical_shape();
    const auto& k_padded = tensor_args.k.padded_shape();
    TT_FATAL(k_logical.rank() == 4, "k must be rank 4");
    TT_FATAL(k_logical[0] == 1, "k dim 0 must be 1");
    TT_FATAL(k_logical[1] == slots, "k slots must match state slots");
    TT_FATAL(k_logical[-2] == TILE, "k row dim must be 32 for full-tile vector bring-up");
    TT_FATAL(k_logical[-1] == state_logical[-2], "k col dim must match state key dim");
    TT_FATAL(k_padded[0] == 1 && k_padded[1] == slots, "k padded slots must match state slots");
    TT_FATAL(k_padded[-2] == TILE, "k padded row dim must be 32");
    TT_FATAL(k_padded[-1] == state_padded[-2], "k padded col dim must match state padded key dim");
...
Qwen36GdnPredictionDeviceOperation::spec_return_value_t Qwen36GdnPredictionDeviceOperation::compute_output_specs(
    const operation_attributes_t& args, const tensor_args_t& tensor_args) {
    if (tensor_args.preallocated_output.has_value()) {
    ...
    return TensorSpec(
        Shape({1, tensor_args.state_scaled.logical_shape()[1], TILE, tensor_args.state_scaled.logical_shape()[-1]}),
        TensorLayout(
...
Qwen36GdnPredictionDeviceOperation::tensor_return_value_t Qwen36GdnPredictionDeviceOperation::create_output_tensors(
    const operation_attributes_t& args, const tensor_args_t& tensor_args) {
    if (tensor_args.preallocated_output.has_value()) {
...
Tensor qwen36_gdn_prediction(
    ...
    using OperationType = ttnn::experimental::prim::Qwen36GdnPredictionDeviceOperation;
