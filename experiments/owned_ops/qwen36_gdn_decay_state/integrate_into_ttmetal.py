#!/usr/bin/env python3
"""Install the owned qwen36_gdn_decay_state op into a TT-Metal checkout.
...
OP_NAME = "qwen36_gdn_decay_state"
...
def patch_transformer_cmake(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        "    qwen36_gdn_decay_state/device/kernels/*\n",
        "transformer kernel glob",
    )
    text = insert_before_once(
        text,
        "            qwen36_gdn_decode/qwen36_gdn_decode.hpp\n",
        "            qwen36_gdn_decay_state/qwen36_gdn_decay_state.hpp\n",
...
def patch_experimental_nanobind(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        '#include "ttnn/operations/experimental/transformer/qwen36_gdn_decay_state/qwen36_gdn_decay_state_nanobind.hpp"\n',
        "experimental nanobind include",
    )
    text = insert_before_once(
        text,
        "    qwen36_gdn_decode::detail::bind_qwen36_gdn_decode(mod);\n",
        "    qwen36_gdn_decay_state::detail::bind_qwen36_gdn_decay_state(mod);\n",
...
def main() -> int:
    ...
    if not (source_dir / "qwen36_gdn_decay_state.cpp").is_file():
