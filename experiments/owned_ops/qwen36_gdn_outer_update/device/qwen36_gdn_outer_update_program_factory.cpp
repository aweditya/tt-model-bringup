...
constexpr uint32_t CB_K_COL = tt::CBIndex::c_1;
constexpr uint32_t CB_DELTA = tt::CBIndex::c_2;
...
Qwen36GdnOuterUpdateProgramFactory::cached_program_t Qwen36GdnOuterUpdateProgramFactory::create(
    const Qwen36GdnOuterUpdateParams& operation_attributes,
    const Qwen36GdnOuterUpdateInputs& tensor_args,
    Tensor& output_tensor) {
    Program program = CreateProgram();

    const auto& state_scaled = tensor_args.state_scaled;
    const auto& k_col = tensor_args.k_col;
    const auto& delta = tensor_args.delta;
    auto* state_buffer = state_scaled.buffer();
    auto* k_col_buffer = k_col.buffer();
    auto* delta_buffer = delta.buffer();
    ...
    const uint32_t total_blocks = slots * key_tiles * value_tiles;
    ...
    create_circular_buffer(program, all_cores, CB_STATE, 2, tensor_tile_size, tensor_format);
    ...
    create_circular_buffer(program, all_cores, CB_STATE_OUT, 2, tensor_tile_size, tensor_format);

    std::vector<uint32_t> reader_compile_time_args = {CB_STATE, CB_K_COL, CB_DELTA};
    ...
    TensorAccessorArgs(delta_buffer).append_to(reader_compile_time_args);

    std::vector<uint32_t> compute_compile_time_args = {CB_STATE, CB_K_COL, CB_DELTA, CB_TMP, CB_STATE_OUT};
    ...
    auto reader_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/transformer/qwen36_gdn_outer_update/device/kernels/dataflow/"
        "reader_qwen36_gdn_outer_update.cpp",
        all_cores,
    ...
    auto writer_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/transformer/qwen36_gdn_outer_update/device/kernels/dataflow/"
        "writer_qwen36_gdn_outer_update.cpp",
        all_cores,
        WriterDataMovementConfig(writer_compile_time_args));

    auto compute_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/transformer/qwen36_gdn_outer_update/device/kernels/compute/"
        "qwen36_gdn_outer_update.cpp",
        all_cores,
        ComputeConfig{
    ...
    Qwen36GdnOuterUpdateSharedVariables shared_variables{
...
void Qwen36GdnOuterUpdateProgramFactory::override_runtime_arguments(
    cached_program_t& cached_program,
    const Qwen36GdnOuterUpdateParams& operation_attributes,
    const Qwen36GdnOuterUpdateInputs& tensor_args,
    Tensor& output_tensor) {
    auto* state_buffer = tensor_args.state_scaled.buffer();
    auto* k_col_buffer = tensor_args.k_col.buffer();
    auto* delta_buffer = tensor_args.delta.buffer();
    auto* output_buffer = output_tensor.buffer();
    ...
    std::vector<std::vector<uint32_t>> reader_runtime_args(shared.cores.size(), std::vector<uint32_t>(7, 0));
    ...
    for (uint32_t i = 0; i < shared.num_cores; ++i) {
        ...
        reader_runtime_args[i][1] = k_col_buffer->address();
        reader_runtime_args[i][2] = delta_buffer->address();
        reader_runtime_args[i][3] = blocks_written;
        reader_runtime_args[i][4] = blocks_per_core;
        reader_runtime_args[i][5] = key_tiles;
        reader_runtime_args[i][6] = value_tiles;
