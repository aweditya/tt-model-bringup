#!/usr/bin/env python3
"""Install the owned qwen36_gdn_outer_update op into a TT-Metal checkout.
...
OP_NAME = "qwen36_gdn_outer_update"
...
def patch_transformer_cmake(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        "    qwen36_gdn_outer_update/device/kernels/*\n",
        "transformer kernel glob",
    )
    text = insert_before_once(
        text,
        "            qwen36_gdn_decode/qwen36_gdn_decode.hpp\n",
        "            qwen36_gdn_outer_update/qwen36_gdn_outer_update.hpp\n",
        "transformer api header",
    )
    text = insert_before_once(
        text,
        "        qwen36_gdn_decode/device/qwen36_gdn_decode_device_operation.cpp\n",
        "        qwen36_gdn_outer_update/device/qwen36_gdn_outer_update_device_operation.cpp\n"
        "        qwen36_gdn_outer_update/device/qwen36_gdn_outer_update_program_factory.cpp\n"
        "        qwen36_gdn_outer_update/qwen36_gdn_outer_update.cpp\n",
...
def patch_ttnn_cmake(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        "qwen36_gdn_outer_update/qwen36_gdn_outer_update_nanobind.cpp\n",
...
def patch_experimental_nanobind(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        '#include "ttnn/operations/experimental/transformer/qwen36_gdn_outer_update/qwen36_gdn_outer_update_nanobind.hpp"\n',
        "experimental nanobind include",
    )
    text = insert_before_once(
        text,
        "    qwen36_gdn_decode::detail::bind_qwen36_gdn_decode(mod);\n",
        "    qwen36_gdn_outer_update::detail::bind_qwen36_gdn_outer_update(mod);\n",
...
def main() -> int:
    ...
    if not (source_dir / "qwen36_gdn_outer_update.cpp").is_file():
