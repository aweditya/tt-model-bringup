#!/usr/bin/env python3
"""Install the owned qwen36_conv1d_decode_owned op into a TT-Metal checkout.

This is a source/build integration helper. It does not open devices and it
does not run TTNN kernels. It copies the op source tree under
`ttnn/cpp/ttnn/operations/experimental/transformer/qwen36_conv1d_decode_owned/`
and patches the three relevant CMake/nanobind registration points so the
op is built + bound the next time ttnn is rebuilt.

Mirrors experiments/owned_ops/qwen36_gdn_decode_owned/integrate_into_ttmetal.py.
"""
...
OP_NAME = "qwen36_conv1d_decode_owned"
...
def patch_transformer_cmake(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = read_text(path)
    # Anchor on the qwen36_gdn_decode_owned entries that we know already
    # exist after the GDN op was installed; that's the natural place to
    # add the conv1d sibling.
    text = insert_before_once(
        text,
        "    qwen36_gdn_decode_owned/device/kernels/*\n",
        "    qwen36_conv1d_decode_owned/device/kernels/*\n",
        "transformer kernel glob (conv1d)",
    )
    text = insert_before_once(
        text,
        "            qwen36_gdn_decode_owned/qwen36_gdn_decode_owned.hpp\n",
        "            qwen36_conv1d_decode_owned/qwen36_conv1d_decode_owned.hpp\n",
        "transformer api header (conv1d)",
    )
    text = insert_before_once(
        text,
        ...
        "        qwen36_conv1d_decode_owned/device/qwen36_conv1d_decode_owned_program_factory.cpp\n"
        "        qwen36_conv1d_decode_owned/qwen36_conv1d_decode_owned.cpp\n",
        "transformer private sources (conv1d)",
    )
    unity_skip = """\
...
"""
    text = insert_before_once(
        ...
        unity_skip,
        "qwen conv1d unity-build exclusion",
    )
...
def patch_experimental_nanobind(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        text,
        '#include "ttnn/operations/experimental/transformer/qwen36_gdn_decode_owned/qwen36_gdn_decode_owned_nanobind.hpp"\n',
        '#include "ttnn/operations/experimental/transformer/qwen36_conv1d_decode_owned/qwen36_conv1d_decode_owned_nanobind.hpp"\n',
        "experimental nanobind include (conv1d)",
    )
    text = insert_before_once(
        text,
        ...
        "experimental nanobind registration (conv1d)",
    )
    write_text(path, text, dry_run)
...
def main() -> int:
    ...
    if not (source_dir / "qwen36_conv1d_decode_owned.cpp").is_file():
