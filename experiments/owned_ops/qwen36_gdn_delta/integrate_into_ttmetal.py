#!/usr/bin/env python3
"""Install the owned qwen36_gdn_delta op into a TT-Metal checkout.
...
def patch_transformer_cmake(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        "    qwen36_gdn_delta/device/kernels/*\n",
        "transformer kernel glob",
    )
    text = insert_before_once(
        text,
        "            qwen36_gdn_decode/qwen36_gdn_decode.hpp\n",
        "            qwen36_gdn_delta/qwen36_gdn_delta.hpp\n",
        "transformer api header",
    )
    text = insert_before_once(
        text,
        "        qwen36_gdn_decode/device/qwen36_gdn_decode_device_operation.cpp\n",
        "        qwen36_gdn_delta/device/qwen36_gdn_delta_device_operation.cpp\n"
        "        qwen36_gdn_delta/device/qwen36_gdn_delta_program_factory.cpp\n"
        "        qwen36_gdn_delta/qwen36_gdn_delta.cpp\n",
...
def patch_ttnn_cmake(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        "qwen36_gdn_delta/qwen36_gdn_delta_nanobind.cpp\n",
...
def patch_experimental_nanobind(tt_metal: Path, dry_run: bool) -> None:
    ...
    text = insert_before_once(
        ...
        '#include "ttnn/operations/experimental/transformer/qwen36_gdn_delta/qwen36_gdn_delta_nanobind.hpp"\n',
        "experimental nanobind include",
    )
    text = insert_before_once(
        text,
        "    qwen36_gdn_decode::detail::bind_qwen36_gdn_decode(mod);\n",
        "    qwen36_gdn_delta::detail::bind_qwen36_gdn_delta(mod);\n",
...
def main() -> int:
    ...
    if not (source_dir / "qwen36_gdn_delta.cpp").is_file():
