...
Qwen36GdnOutputProgramFactory::cached_program_t Qwen36GdnOutputProgramFactory::create(
    const Qwen36GdnOutputParams& operation_attributes,
    const Qwen36GdnOutputInputs& tensor_args,
    Tensor& output_tensor) {
    Program program = CreateProgram();

    const auto& state_next = tensor_args.state_next;
    const auto& q = tensor_args.q;
    auto* state_buffer = state_next.buffer();
    auto* q_buffer = q.buffer();
    auto* output_buffer = output_tensor.buffer();

    const uint32_t slots = state_next.logical_shape()[1];
    const uint32_t key_tiles = state_next.padded_shape()[-2] / TILE;
    const uint32_t value_tiles = state_next.padded_shape()[-1] / TILE;
    const uint32_t total_blocks = slots * value_tiles;

    const bool row_major = true;
    auto grid_size = state_next.device()->compute_with_storage_grid_size();
    const auto
    ...
    const tt::DataFormat tensor_format = datatype_to_dataformat_converter(state_next.dtype());
    ...
    create_circular_buffer(program, all_cores, CB_OUTPUT, 2, tensor_tile_size, tensor_format);

    std::vector<uint32_t> reader_compile_time_args = {CB_STATE_IN, CB_Q};
    ...
    std::vector<uint32_t> compute_compile_time_args = {CB_STATE_IN, CB_Q, CB_OUTPUT};
    ...
    auto reader_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/transformer/qwen36_gdn_output/device/kernels/dataflow/"
        "reader_qwen36_gdn_output.cpp",
        all_cores,
    ...
    auto writer_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/transformer/qwen36_gdn_output/device/kernels/dataflow/"
        "writer_qwen36_gdn_output.cpp",
        all_cores,
        WriterDataMovementConfig(writer_compile_time_args));

    auto compute_kernel_id = CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/experimental/transformer/qwen36_gdn_output/device/kernels/compute/"
        "qwen36_gdn_output.cpp",
        all_cores,
        ComputeConfig{
    ...
    Qwen36GdnOutputSharedVariables shared_variables{
...
void Qwen36GdnOutputProgramFactory::override_runtime_arguments(
    cached_program_t& cached_program,
    const Qwen36GdnOutputParams& operation_attributes,
    const Qwen36GdnOutputInputs& tensor_args,
    Tensor& output_tensor) {
    auto* state_buffer = tensor_args.state_next.buffer();
    ...
    const uint32_t key_tiles = tensor_args.state_next.padded_shape()[-2] / TILE;
    const uint32_t value_tiles = tensor_args.state_next.padded_shape()[-1] / TILE;
