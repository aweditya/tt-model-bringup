...
std::vector<Tensor> qwen36_topk_owned(
    ...
    const uint32_t adjusted_k =
        operations::experimental::qwen36_topk_owned::CMAKE_UNIQUE_NAMESPACE::get_nearest_supported_k_value(k, tile_width);
    ...
    if (preallocated_output_tensors.has_value()) {
        const auto preallocated_values = operations::experimental::qwen36_topk_owned::CMAKE_UNIQUE_NAMESPACE::pre_topk_transform_tensor(
            std::get<0>(preallocated_output_tensors.value()), dim, is_dim_last_idx, is_rank_le_4d);
        const auto preallocated_indices =
            operations::experimental::qwen36_topk_owned::CMAKE_UNIQUE_NAMESPACE::pre_topk_transform_tensor(
    ...
    auto [output_value_tensor, output_index_tensor] = ttnn::prim::qwen36_topk_owned(
        padded_tensor,
        adjusted_k,
        -1,
        largest,
    ...
    auto post_transform_output_tensors =
        operations::experimental::qwen36_topk_owned::CMAKE_UNIQUE_NAMESPACE::post_topk_transform_tensor(
            transposed_tensor,
            output_tensor_vec,
            dim,
            is_dim_last_idx,
