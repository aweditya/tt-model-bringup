...
#include "ttnn/operations/experimental/transformer/qwen36_topk_owned/device/qwen36_topk_owned_device_operation.hpp"
...
#include "ttnn/operations/experimental/transformer/qwen36_topk_owned/device/qwen36_topk_owned_constants.hpp"
#include "ttnn/operations/experimental/transformer/qwen36_topk_owned/device/qwen36_topk_owned_utils.hpp"
 ...
 *
 * 4. MEMORY AND CORE CONSTRAINTS: Pass qwen36_topk_owned_verify_multi_core_cost() checks
 ...
 */
Qwen36TopkOwnedDeviceOperation::program_factory_t Qwen36TopkOwnedDeviceOperation::select_program_factory(
    ...
    if (multicore_supported) {
        ...
        multicore_supported &= qwen36_topk_owned_verify_multi_core_cost(
    ...
    if (multicore_supported) {
        return Qwen36TopkOwnedMultiCoreProgramFactory{};
    }
    return Qwen36TopkOwnedSingleCoreProgramFactory{};
}

void Qwen36TopkOwnedDeviceOperation::validate_on_program_cache_miss(
    const operation_attributes_t& args, const tensor_args_t& tensor_args) {
    const auto& input_tensor = tensor_args.input;
    ...
    if (input_shape[args.dim] >= ttnn::prim::constants::multi_core_min_width) {
        ...
        can_run = qwen36_topk_owned_verify_multi_core_cost(
        ...
        if (!can_run) {
            can_run = ttnn::prim::qwen36_topk_owned_verify_single_core_cost(input_tensor, args.k, uint16_output);
        }
    } else {
...
Qwen36TopkOwnedDeviceOperation::spec_return_value_t Qwen36TopkOwnedDeviceOperation::compute_output_specs(
    const operation_attributes_t& args, const tensor_args_t& tensor_args) {
    const auto& input_tensor = tensor_args.input;
    const auto& preallocated_outputs = tensor_args.preallocated_outputs;
...
Qwen36TopkOwnedDeviceOperation::tensor_return_value_t Qwen36TopkOwnedDeviceOperation::create_output_tensors(
    const operation_attributes_t& args, const tensor_args_t& tensor_args) {
...
std::tuple<ttnn::Tensor, ttnn::Tensor> qwen36_topk_owned(
    ...
    return ttnn::device_operation::launch<Qwen36TopkOwnedDeviceOperation>(
        Qwen36TopkOwnedParams{
            .k = k,
            .dim = dim,
            .largest = largest,
            .sorted = sorted,
