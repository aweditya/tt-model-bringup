 ...
 *
 * Examples:
 * - qwen36_topk_owned_largest_power_of_two(15) = 8  (2^3)
 * - qwen36_topk_owned_largest_power_of_two(16) = 16 (2^4)
 * - qwen36_topk_owned_largest_power_of_two(100) = 64 (2^6)
 */
uint32_t qwen36_topk_owned_largest_power_of_two(uint32_t x) { return x == 0 ? 0 : (1U << (31 - __builtin_clz(x))); }
 ...
 * @return Optional Qwen36TopkOwnedCoreConfig with optimal settings, or nullopt if impossible
 */
std::optional<Qwen36TopkOwnedCoreConfig> find_qwen36_topk_owned_core_config(
    uint32_t width,
    uint32_t min_dim,
    uint32_t max_dim,
    uint32_t k,
    ...
    const uint32_t start_split_size =
        static_cast<uint32_t>(width / tile_width / qwen36_topk_owned_largest_power_of_two(max_cores)) * tile_width;
    ...
    for (uint32_t split_size = start_split_size; split_size <= max_dim; split_size *= 2) {
        ...
        if (num_cores <= max_cores &&                                                        // Core count feasible
            ...
            Qwen36TopkOwnedCoreConfig config{};
 ...
 *
 * This is a convenience function that wraps find_qwen36_topk_owned_core_config to provide
 ...
 */
bool qwen36_topk_owned_verify_multi_core_cost(
    uint32_t width,
    uint32_t min_dim,
    uint32_t max_dim,
    uint32_t k,
    ...
    uint32_t index_tile_size,
    uint32_t tile_width) {
    const auto config = find_qwen36_topk_owned_core_config(
 ...
 */
bool qwen36_topk_owned_verify_single_core_cost(const ttnn::Tensor& input_tensor, uint32_t k, bool uint16_output) {
