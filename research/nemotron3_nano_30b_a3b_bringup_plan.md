...
| **v0.1.4** | **MoE Expert-Parallel refactor (True EP, DeepSeek-V3-style)**. 128 experts sharded as 32/chip via `ShardTensorToMesh(dim=0)`. Forward = pre-norm → router → `all_to_all_dispatch(output_concat_dim=2)` → 32 local experts → `all_to_all_combine(output_shard_dim=2)` → shared expert (replicated) → residual. Pad S=5→8 (next multiple of NCHIPS=4); shard seq dim 2 via `ShardTensorToMesh(dim=2)`; dispatch_out per-chip `[1, 1, 8, 2688]`; combine_out per-chip `[6, 1, 2, 2688]`; readback `ConcatMeshToTensor(dim=2)` → full `[6, 1, 8, 2688]`; slice routed_np back to S_orig=5. Bootstrap 9.2s; forward 1.8s. Per-chip compute drops 128 → 32 experts. Memory ≤ 7.8 GB/chip → unblocks v0.2+. | Gate S shared_out cos=0.999713 ✓; Gate M mixer_out cos=0.999826 ✓; Gate B block_out cos=0.999805 ✓ (bit-equivalent to v0.1.3.b naive Pattern A at 6-digit precision) | ✅ DONE (commit `1abce07`) — True EP path live |
| **v0.2** | All 52 layers (per-layer dispatch via `state.layer_types[L]`) + final_norm + lm_head + argmax. v0.2.a: L0..L5 chained (one of each block kind) — 6/6 cos ≥ 0.999. v0.2.b: full 52-layer streamed (upload→run→deallocate per layer to fit 8 GB) → final_norm cos=0.9945 → argmax_last match HF. Streaming infrastructure: `state.key_to_shard`, `srv.upload_one_layer`, `srv.deallocate_layer`. Peak memory = top-level (~1.4 GB) + 1 layer (~640 MB max). | v0.2.a 6/6 PASS (cos ≥ 0.999); v0.2.b TT argmax_last = HF argmax_last = 6993 ✓ | ✅ DONE (commits `e54481c`, `5ffd183`) |
| **v0.2.5** | **On-device tensor flow refactor**. Add `attn_block_eager_tt`, `mamba2_block_eager_tt`, `moe_block_eager_ep_tt` variants taking + returning `ttnn.Tensor`. h_tt persists across all 52 blocks; intra-block bridges (SSD wrapper, router topk, combine sum) deferred to v0.5. Perf finding: 82.4s vs v0.2.b's 77.7s — essentially same, because the 52 inter-block round trips (~10ms each) were drowned by ~3s/layer MoE weight upload (~69s of 82s = disk I/O). Value is **architectural enabler** for v0.3 (state-carry on device) + v0.4 (trace requires on-device flow) + v0.5 (clean op chain to fuse). For v0.3 multi-step decode the real perf bottleneck shifts to weight streaming — needs in-memory residency (v0.5 territory). | TT argmax_last == HF argmax_last == 6993 ✓; 0 inter-block readbacks | ✅ DONE (commit `926b49c`) |
| **v0.2.6** | **Host-side weight cache** — module-level `_WEIGHT_NP_CACHE` + state.weight_np_cache. Also caches pre-stacked + pre-transposed MoE expert blob (saves 128×np.stack + .T per warm iter). Honest finding: original 25s target was wrong; the OS file cache was already warm, so disk I/O wasn't the bottleneck. The dominant cost is `ttnn.from_torch(...)` (bf16 cast + tilize + shard + DMA to 4 chips), which no host-side cache can avoid. Got iter 2 = 61.8s (2.97× warm speedup vs cold iter 1 = 183.8s). | argmax_last == 6993 on both iters ✓; warm/cold speedup 2.97× | ✅ DONE (commit `1685827`) |
| **v0.3.0** | **All-layers-resident bootstrap** — `NEMOTRON3_UPLOAD_LAYERS=all` sentinel mirrors `server_35b_ttnn.py:1818`. P150 has 32 GB/chip (verified `ttnn/core/operation.cpp:33`); full Nemotron-3 Nano load ≈ 21 GB/chip — fits easily. Streaming pattern was over-engineered for a memory problem that doesn't exist. Bootstrap heartbeat (5s cadence) + tee'd log for ssh-drop survival. | bootstrap 106.5s; iter 1 cold 17.4s; iter 2 warm 16.7s; argmax = HF = 6993 ✓; **3.69×** vs v0.2.6 warm | ✅ DONE (commit `42d303d`) |
| **v0.3.2** | **Nemotron-3 dev harness** — `nm3_dev_harness.py` + `run_harness_tmux.sh nm3`. Bootstrap once (108s), iterate via `touch .cache/nm3_runtime/trig/<test>` → ~10s/iter instead of 108s. Forks the cb35/gm4 pattern. Smokes that accept `state=None` (like `nemotron3_v030_resident_smoke.py`) work via the harness. | smoke runs through harness without re-bootstrap; per-iter ~17s vs ~120s standalone | in_progress |
| **v0.3.1.a** | **Quadratic-prefix multi-step decode smoke** — re-runs full forward on growing prefix each step (O(n²)). No KV cache or ssm_state plumbing needed; validates multi-step CORRECTNESS first. Forks v0.3.0 forward. Iterates via the nm3 harness. HF oracle re-generated with `--gen 8`. Result: **7/8 PASS** — TT matched HF's `Paris, Paris, Paris, ...` loop exactly for steps 0-3, drifted at step 4 (TT=5498 vs HF=6993, single argmax flip), then RECOVERED to match for steps 5-7. The recovery is strong evidence the model is semantically correct — even with a different prefix after step 4, TT independently locked back onto the same loop. **Drift cause: bf16 matmul precision** (not state management; v0.3.1.b won't fix it). Step time: 17-28s per forward. | 7/8 token-for-token (drift event at step 4, recovered after) | ✅ DONE (commit `f1fd079`) |
| **v0.3.1.b** | **Mamba2 ssm_state plumbing (forward-only)**. `state.ssm_state_np[L]` lazy-init via `reset_decode_state()`; `mamba2_block_eager_tt` reads + writes if populated, falls back to fresh zeros if not (no-op for v0.3.0/v0.3.1.a). Defensive `getattr` for harness state-version skew. | v0.3.0 regression PASS (argmax=6993) | ✅ DONE (commit `5ca94b8`) |
| **v0.3.1.c** | **Paged decode for attention** — fork verbatim from 35B/27B/Gemma 4 per [audit](nemotron3_state_caching_audit_2026-06-05.md). Critical: NUM_KV_HEADS=2 + NCHIPS=4 forces **Gemma 4's two-call pattern** (one cache per KV head, NKV_per_chip=1) — single-call fails `paged_update_cache` dim[1] check. Per-cache shape `[NUM_BLOCKS=256, NCHIPS=4, BLOCK_SIZE=32, HEAD_DIM=128]` bf16 TILE sharded dim=1. **Split** into `attn_prefill_tt` (S>1, paged_fill_cache + non-paged SDPA) and `attn_decode_step_tt` (S=1, paged_update_cache + paged SDPA decode) — trace-cleaner for v0.4. **Step-by-step**: 1/3 ✅ (`f4743c6` State slots), 2/3 ✅ (`840d584` setup_paged_decode_state, 12/12 gates green), 3a ✅ (`b7af8cf` attn_prefill_tt writes cache, argmax=6993 PASS), 3b ✅ plumbing (`abe079b` decode runs end-to-end without crash, argmax=1911 ≠ HF 1063 → correctness debug needed). | 8-token decode matches v0.3.1.a 7/8; per-step time ~constant | plumbing ✅; correctness debug pending |
| **v0.3.1.c step 3c** | **Correctness debug — DIAGNOSED 2026-06-05 via teacher-forced ladder**. Ladder PASS/FAIL: **attention 6/0 ✓** (paged decode is correct end-to-end), **moe 22/1** (likely bf16 noise crossing 0.99), **mamba2 14/9** with **L0 FIRST FAIL cos=0.860 mad=2.6e-2**. Root cause: **Mamba2 conv_state isn't being carried** between prefill and decode. Conv1d kernel size = 4. Prefill (S=5): conv internally uses 3-position pad correctly. Decode (S=1): conv on 1 new token with zero pad LOSES previous 3 positions of x_BC. mamba2 has TWO state components: ssm_state (carried ✓) + conv_state [3, conv_dim=6144] (NOT carried ✗). Probe: `nemotron3_v031c_decode_ladder_smoke.py`. | first-fail layer identified, fix scope clear | ✅ diagnosed |
| **v0.3.1.c step 3d** | **Fix Mamba2 conv_state carry**. Added `state.conv_state_np[L]` ([B, CONV_KERNEL-1=3, CONV_DIM_M=6144] fp32) lazy buffer; `mamba2_block_eager_tt` branches: DECODE prepends conv_state to xBC then conv1d(pad=0); PREFILL uses on-device padded conv1d but saves last 3 positions for next call. | **Ladder: mamba2 23/0 PASS** (was 14/9); decode_smoke argmax = HF = 1063 ✓ end-to-end | ✅ DONE (commit `8ff3c57`) |
| **v0.3.1 COMPLETE** | End-to-end constant-time single-token decode: prefill 5 tokens → carry ssm_state, conv_state, KV cache → decode 1 token → TT argmax matches HF byte-exact. Per-step decode: ~15s cold (JIT compile); subsequent traced/warm decodes would be ms-scale. | **PREFILL argmax = 6993 ✓ ; DECODE argmax = 1063 = HF ✓** | ✅ DONE (commit `8ff3c57`) |
| **v0.3.3** | **N-step decode chain via state carry** — prefill 5 tokens once, then loop N decode steps with cur_pos advancing, ssm_state + conv_state + KV cache all evolving. | **6/7 PASS with recovery**: TT chain `[1063, 6993, 1063, 6993, 1063, 5498, 1063]` vs HF `[1063, 6993, 1063, 6993, 1063, 6993, 1063]` — identical bf16 drift pattern to v0.3.1.a (single flip at step 5, recovered at step 6). cur_pos advances 5→11 correctly. Constant-time decode FULLY VALIDATED. | ✅ DONE (commit `a617a76`) |
| **v0.3.3.b** | **JIT-warm decode perf characterization** (data scopes v0.4). 5 decode steps measured: cold step 0 = 15.48s, warm mean = 15.469s, **JIT overhead 0.01s, cold/warm = 1.0× (no JIT win)**. 15.5s/step is essentially all host-bridge time: 23 Mamba2 layers × ~12 host roundtrips in SSD wrapper ≈ 14s; MoE host topk + combine sum + residual ≈ 1.5s. Tok/s = 0.065 (about 470× slower than the 30 tok/s demo target). **Trace is required** — confirmed not optional. | data drove v0.4 scope decision | ✅ DONE (commit `a4060de`) |
| **v0.3** | (umbrella) Phase 1 complete when v0.3.1 PASSES. | (rolled up) | in_progress |

### Phase 2 — Single-stream performance (v0.4 → v0.5)

| Stage | Adds | Gate | Status |
|---|---|---|---|
| **v0.4.0a** | Pre-upload SSD constants. PASS correctness (cos=1.0), perf delta tiny (46ms/step out of 15500ms = 0.3%). Kept as foundation but not the hot spot. | cos=1.0 across 10 calls; speedup 1.15× | ✅ DONE (commit `81c98c6`) |
| **v0.4.0b** | **Section profiler** for mamba2_block_eager_tt. Decisive finding: **S2 (conv1d + readback + reupload) is 97% of layer time** (637/656 ms). All other sections combined: 19ms. **The v0.4.0a-d ladder targeted the wrong subsystem.** | profile data localises bottleneck | ✅ DONE (commit `243a7ec`) |
| **v0.4.0c** | **DONE — on-device conv_state, no numpy roundtrip**. state.conv_state_tt ttnn tensor; xBC stays on device; ttnn.concat prepends; conv1d → reshape → to_layout(TILE) all on device. Bug found via TT_THROW: ttnn.reshape returns a view, must materialise via to_layout BEFORE deallocating source. Correctness preserved (1063→6993→1063→6993→1063 = HF). **Perf finding: per-step time UNCHANGED at 15.5s**. The original S2=637ms was mostly the ttnn.conv1d KERNEL TIME on (1,4) mesh, not the host bridges. So v0.4.0c lands the architectural hygiene (matches "on device only" directive) but doesn't move the perf needle. | argmax chain matches HF; numpy bridges gone | ✅ DONE (commit `9db5909`) |
| **v0.4.0d** | **Conv1d-alone isolation probe.** Confirmed ttnn.conv1d on [B=1,1,4,6144] is 651 ms warm in isolation — kernel itself, not host bridges. Trace won't help. Conclusion: rewrite the depthwise k=4 conv as element-wise math. | warm conv1d call time + verdict | ✅ DONE (commit `c3a47e1`) |
| **v0.4.0e** | **MATMUL-FOLD — replaces ttnn.conv1d with 4×mul + 3×add + 1×bias-add.** Math: `y[c] = Σ_{k=0..3} w_k[c] * x[k,c] + b[c]`. Per-position weights split at upload (4 × [1,1,1,6144] TILE + bias TILE on mesh). Decode hot path slices [B,1,4,C] combined into 4 views, mul each by w_k, accumulate, add bias. Gated on S==1; prefill keeps ttnn.conv1d. **Isolation: 656 → 1.9 ms (345× kernel-only, cos=0.999994 vs conv1d). Integration: per-step time 15.5s → 0.6-0.7s warm (~22× full-step speedup).** N-step chain regression unchanged (6/7 PASS, same step-3 bf16 flip as baseline). | warm step time ≤ 1s + n-step chain ≥ 6/7 | ✅ DONE (commits `88328e9` probe + `425acad` integration) |
| **v0.4.0f** | **Per-layer-kind decode profiler** (forks v0.4.0b method). Localises the next bottleneck on the post-fold step. Found: **mamba2 = 57%** (379 ms across 23 layers); moe = 36%; attn = 2%; embed+lm_head = 4%. Confirms the trace-plan hypothesis (SSD wrapper numpy roundtrip dominates). Drives v0.4.0g. | per-kind ms localised | ✅ DONE (commit `143ca85` probe + `9c2c5c3` HANDOFF) |
| **v0.4.0g.a** | **Pure-state Mamba2 SSD wrapper**. New `mamba2_decode_step_ttnn_pure_state` takes `ssm_state_tt` (ttnn fp32 mesh tensor) instead of numpy AND returns `(state_out_tt, y_out_tt)` instead of numpy. Eliminates the 2 MB per-layer per-step ssm_state roundtrip + the y readback/re-upload. Plus: `mixer_norm_w_tt` and SSD constants (`dt_bias_tt`, `A_log_tt`, `D_tt`) pre-uploaded at `upload_mamba2_layer`. `state.ssm_state_tt[L]` allocated in `reset_decode_state` (fp32 zeros, mesh-replicated). Gated on `S == 1` (decode); prefill keeps the legacy numpy path. **Deferred to v0.4.0g.b**: x/z/B/C/dt still readback as numpy → padded → re-upload (small data). Lifts the highest-ROI subset of #223. Forks 35B's `qwen36_gdn_decode_owned` pattern at `server_35b_ttnn.py:580-607`. | n-step chain ≥ 6/7 PASS + measurable mamba2 share drop | in progress |
| **v0.4.0g.b** | **On-device input padding**. Replace `_pad_per_head_vector`, `_pad_dt_per_batch_per_head`, `_replicate_per_group_to_per_head` with ttnn-pure equivalents (permute + pad + repeat). Finishes the user's "make it solely operating with TT stuff" mandate. Inputs x/z/dt/B_in/C_in flow straight from `silu_out_tt` to kernel without readback. | n-step chain ≥ 6/7 + mamba2 ~3-5 ms/layer | pending |
| **v0.4.0h.a** | **On-device MoE combine weighted-sum**. all_gather(combine, dim=2) + ttnn.mul(broadcast) + ttnn.sum(dim=0) + slice replaces the host weighted-sum bridge (was the largest readback in MoE, ~12 MB/step). Reuses 27B/35B precedents. **Result**: 325 → 264 ms warm step (-19%); MoE per-layer 8.2 → 5.8 ms (-29%); n-step chain 7/7 PASS unchanged. | n-step chain ≥ 6/7 + moe share drop | ✅ DONE (commit `fe502e3`) |
| **v0.4.0h.b** | **On-device router + h reshard** — split into sub-probes. (i) router on-device probed at `nemotron3_v040hb_ondevice_router_probe.py`: cos=0.9997, 6/8 expert sets match — tie-break drift via `ttnn.topk` vs `numpy.argpartition`. Park until long-context baseline lands. (ii) h reshard — superseded by v0.4.1.e below. | n-step chain ≥ 6/7 + drift acceptable at long context | router PARKED; reshard DONE via v0.4.1.e |
| **v0.4.1.e** | **`ttnn.reduce_scatter` + 1/N scale solves replicate→shard**. User's intuition rescued blocker #3 from the false-negative declared at v0.4.1.c+d. Correct API: `ttnn.reduce_scatter(input, dim, cluster_axis=...)` with hardcoded sum-reduce (no `math_op` kwarg — my v0.4.1.c probe used a fake kwarg). Math: pre-scale by 1/N on each chip → reduce_scatter sums the 4 identical copies (= h) and scatters along dim 2. Validated cos=0.999999 in `nemotron3_v041e_reduce_scatter_correct_probe.py`. Integration: 16-line diff in `moe_block_eager_ep_tt` replacing the `ttnn.to_torch`→numpy→`ttnn.from_torch` host bridge with `ttnn.multiply` + `ttnn.reshape` + `ttnn.reduce_scatter` + `ttnn.to_layout`. n-step chain 7/7 PASS, 0.2s warm step retained (no perf regression). | n-step chain ≥ 6/7 + zero `_rb` / `ttnn.to_torch` in the resharder branch | ✅ DONE (commits `4c3cd9b` probe + `a65af53` integration + `5e294aa` docs) |
| **v0.4.1.f / .g** | **Nemotron-3 needle baseline DONE 2026-06-05.** Result: 0/4 retrieval at L=128 + L=512 (both MAX_NEW=24 and 120). BUT text is COHERENT not gibberish ("The user wants to..." / "The question is:..."). Same text patterns at both horizons — extending didn't help. Per Gemma 4 Round 9 lesson: prompt-shape failure, NOT correctness (v0.3.3 already proves correctness via 7/7 token-for-token chain at the Paris prompt). **Strategic consequence**: deterministic text outputs ARE the drift gate — after router swap, identical text = benign drift. Tracks text-output diff, not retrieval rate. | text patterns logged for next-step comparison | ✅ DONE (commits `349639e`) |
| **v0.4.0h.b.router** | **MoE router on-device — INTEGRATION ATTEMPTED, FAILED. (commit `29234f5`)** Env-gated `NM3_ROUTER_ON_DEVICE=1`. Implemented `ttnn.topk + ttnn.embedding`-gather with lazy bias-tensor upload. Default OFF preserves 7/7 chain PASS. With env ON: **0/7 chain FAIL** from decode step 0 — every token wrong. The probe's "cos=0.9997 / 6/8 sets" drift compounds across 23 MoE layers per forward into immediate argmax divergence. **Conclusion**: `ttnn.topk` tie-breaking differs from `numpy.argpartition` in a way that's not perceptible at single-layer cos but lethal at full-network scale. Need a tie-break-stable on-device topk — options: (a) `ttnn.sort + slice` (deterministic, more expensive); (b) pre-bias scores by tiny per-expert epsilon to enforce a unique ordering; (c) find a ttnn.topk variant matching numpy's tie-break. | n-step chain ≥ 6/7 with router ON | ❌ FAILED via direct integration; env-gate kept for future variants |
| **v0.4.0h.c.research** | **Root cause confirmed (commit `12f101b`)**. HW SFPSWAP comparison is unstable (tt-metal#20625). PR #31989 added stable-sort flag at LLK level but `ttnn.topk` doesn't expose it; bug #33492 (`ttnn.sort(stable=True)`) still open. `ttnn.topk` hard-asserts bf16 input — no fp32 promotion. Empirical: seed-99 distribution has 6/8 rows with EXACT bf16-quantized ties at the K-th rank — any topk variant must pick from tied K-th candidates. Host-vs-device gap is NOT a topk bug; it's precision asymmetry in the ADD (host: bf16-readback + fp32 bias = 8/8 match; device: bf16 + bf16 = 6/8 match). **HF itself uses `torch.topk(sorted=False)` (modeling_nemotron_h.py:905)** which is non-deterministic on CUDA — there is no "correct" reference. **DeepSeek-V3 demo's source default is `topk_fallback=False`** (`moe_gate.py:126`, `moe.py:163,288,305`) — device-topk IS the production default; the host fallback is a TEST path (`test_moe.py:274` pins True). They accept the drift because demo uses `DEFAULT_SAMPLING_TEMPERATURE` (sampling washes out tie-breaks). Our chain uses greedy argmax → exposes drift. **VERDICT**: keep host topk for our greedy-argmax gate; if we ever switch to sampling-temperature production, device-topk becomes shippable. Trace blocker #2 stays "blocked on upstream" for the deterministic-gate use case. | research durable | ✅ DONE (commit `12f101b` + correction in `[next]`) |
| **v0.4.0h.d** | **Long-context coherence test, router on-device.** Re-ran v041f needle baseline (4 prompts × 24 tok max_new) with `NM3_ROUTER_ON_DEVICE=1`. Result: 3/4 prompts produce **gibberish** at router=ON (`': pleeer? pleeer? p'`, `'edet1? Actually, ed'`, `' __________ ______'`) vs baseline's coherent reasoning. 1/4 trials (L=512 needle BMK7DKCD) coincidentally matched baseline (`'\nThe question is:'`). PLUS router=ON is **slower** (254 vs 220 ms/tok mean decode) — no perf benefit AND broken text. Definitive verdict: router on-device is NOT shippable at greedy argmax. | text-diff vs baseline | ❌ FAIL (text broken + perf regression) |
| **v0.4.0h.e.refactor** | **Move `cur_pos_buf` update OUT of `attn_decode_step_tt`** (server_nemotron3_nano_ttnn.py:1342-1343). v0.4.1.h probe empirically discovered the 5th host write: `_set_cur_pos_buf → ttnn.copy_host_to_device_tensor` fires 6× per decode step inside captured region → TT_FATAL. Fix: caller updates cur_pos_buf once per step before `execute_trace` (mirrors update_tok_buf pattern). Server refactor only; ~1 hour. | trace capture passes with router=ON + cur_pos_buf update outside capture | pending (server-side trace prereq) |
| **v0.4.0h.e** | **Owned `qwen36_topk_owned` kernel that flips the LLK stable-sort flag.** Forks the `qwen36_gdn_decode_owned` / `qwen36_decay_gate_decode_owned` owned-op pattern (already in qb1+qb2 builds per `[[reference-remote-host]]`). Path: copy `ttnn::topk` op + program factory; thread `stable=true` down to the LLK `topk_st8` descent (PR #31989 LLK flag exposed). Validation: 7/7 chain regression at greedy argmax — passes only if the deterministic order matches numpy.argpartition closely enough across 23 MoE layers. Known caveat: kernel hard-asserts bf16/bfp8 input (`topk_device_operation.cpp:147-149`); stable-sort fixes order-determinism but bf16 numerical ties remain. If 7/7 fails post-stable-flag, follow-up `qwen36_topk_owned_fp32` that promotes input to fp32 before the descent. **Unlocks trace** (blocker #2) → projected 0.26s eager → ~100-130 ms traced (8-10 tok/s, ~2.5-3× speedup based on 27B/35B trace ratios). Effort: ~1-2 days. | n-step chain 7/7 with owned topk ON + traced perf measured | pending (HIGHEST ROI lever) |
| **v0.4.1** | First trace capture — gated on v0.4.0h.e. If owned topk lands, trace unblocks immediately. Eager fallback stays at 0.26s warm step (~3.8 tok/s steady, 5.0 peak). | 100 traced steps == 100 eager token-for-token | gated on v0.4.0h.e |
| **v0.4.1** | First trace capture of single decode step. All 4 blockers cleared once router lands (#1 embed, #2 router, #3 replicate→shard via v0.4.1.e, #4 final_norm/lm_head/argmax). | 100 traced steps == 100 eager token-for-token | unblocked after router |
...
| **v0.5.P1** | **Vocab-shard lm_head + on-device argmax (commit `693806a`).** Forks 27B P22 (server_tp.py:399 + :1680-1688). Per-chip lm_head matmul produces [B, S, VOCAB_PER_CHIP=32768]; all_gather replicates to [B, S, VOCAB=131072]; on-device argmax → uint32 [B, S, 1] (8-byte readback vs 262 KB pre-shard). **MEASURED 2026-06-07 on fresh qb1 bootstrap: 260 → 203.1 ms/tok eager (-21.9%, 4.92 tok/s, +28%). Beat target by 17 ms; bigger than 27B P22 +5-8% because Nemotron-3's smaller HIDDEN (2688) makes lm_head a larger fraction of step time.** 30-step measurement, prefill argmax=6993 PASS, chain matches v0.3.3 baseline. | warm step ≤ 245 ms + chain regression preserved | ✅ DONE |
