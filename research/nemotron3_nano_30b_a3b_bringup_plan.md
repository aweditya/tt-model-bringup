...
| **v0.1.2.c** | **L0 — full forward (on-device chain).** Implements pre-norm → in_proj → split → conv1d → causal slice → silu → split x/B/C → host-orchestrated SSD loop (each call hits the on-device kernel via the G4 wrapper) → MambaRMSNormGated (group-RMSNorm → weight → silu(z)) → out_proj → residual. Wrapper patched for mesh-awareness (`isinstance(device, ttnn.MeshDevice)` branches the upload/readback). | Norm 0.957; o_proj 0.981; mixer_out 0.981; block 0.982 — all below the 0.999 gate. **Root cause**: even with HF-correct inputs + pure numpy fp32 reference, cos to HF L0_norm is only 0.957373 (norm-then-gate) or 0.838 (gate-then-norm). Our `mamba2_numpy_oracle.mamba2_decode_step` has a math drift vs HF `mamba_chunk_scan_combined`. The on-device kernel matches our oracle (cos=0.999998 in isolation); the oracle drifts from HF. Need a focused single-position SSD oracle-vs-HF probe to close. | 🟡 PARTIAL (commit `587ae06`); v0.1.2.d next |
| **v0.1.2.d** (new) | **Single-position SSD oracle vs HF** — feed HF L0_in_proj + L0_conv1d (post-causal-slice, post-silu) split into x/B/C/dt, run our numpy oracle for ONE position, compare against what HF emits at that step. Localize the exact math drift (dt path? A_log handling? D term? GQA broadcast?). Once oracle matches HF, kernel + wrapper auto-match → v0.1.2.c gates flip to PASS. | numpy ref(single-step) vs HF stepwise ≥ 0.999 | pending |
