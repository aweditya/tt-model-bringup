...
| **v0.2.6** | **Host-side weight cache** (DECIDED 2026-06-05). v0.2.5 measurement: 69s/82s = MoE disk I/O. Add a `state.weight_np_cache` dict; `load_t(state, key)` checks cache before re-reading safetensors. Steady-state iter 2+: ~20s/step (4× speedup) vs iter 1 cold. Required before multi-step decode is bearable. ~30 lines. | iter-2 step time ≤ 25s; argmax_last still = 6993 | pending |
