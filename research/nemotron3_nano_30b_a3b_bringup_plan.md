...
| **v0.3.1.c step 3d** | **Fix Mamba2 conv_state carry**. Add `state.conv_state_np[L]` (numpy [3, CONV_DIM_M=6144] fp32) populated lazily by `reset_decode_state`. Modify `mamba2_block_eager_tt`: (a) read conv_state if set, prepend to x_BC before conv1d; (b) save last 3 positions of x_BC to conv_state after conv1d. For prefill (S=5): no prepend, but save last 3 positions for next call. For decode (S=1): prepend conv_state (3) → input becomes (4) → conv produces 1 output, then save 3 positions for next. ~30 LOC. Re-run ladder; mamba2 cos should jump to ≥ 0.99 across all 23 layers. | mamba2 23/0 PASS in ladder; decode_smoke argmax matches HF gen[6]=1063 | pending |
