...
| **v0.2.5** | **On-device tensor flow refactor** (DECIDED 2026-06-05 — gates v0.3+v0.4). The block_eager_*` functions currently take numpy in, return numpy out → 52 host round trips per forward (~2 MB of bridging per step). Multi-step decode at 77s/step (v0.2.b measurement) is impractical for needle-haystack (L=8192 → 7+ days/run). Refactor: add `attn_block_eager_tt`, `mamba2_block_eager_tt`, `moe_block_eager_ep_tt` variants taking + returning `ttnn.Tensor`. Streaming weight pattern (`upload_one_layer → run → deallocate_layer`) is ORTHOGONAL — h_tt stays resident across the layer, weights swap in/out. Regression gate: re-run v0.2.b smoke through the new tt-flow path → argmax_last == HF == 6993. | v0.2.b regression PASS + measured per-step time drop (target ≥5× faster eager) | pending |
