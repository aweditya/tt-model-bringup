...
| **v0.3.1.c** | **Paged decode for attention** — fork verbatim from 35B/27B/Gemma 4 per [audit](nemotron3_state_caching_audit_2026-06-05.md). Critical: NUM_KV_HEADS=2 + NCHIPS=4 forces **Gemma 4's two-call pattern** (one cache per KV head, NKV_per_chip=1) — single-call fails `paged_update_cache` dim[1] check. Per-cache shape `[NUM_BLOCKS=256, NCHIPS=4, BLOCK_SIZE=32, HEAD_DIM=128]` bf16 TILE sharded dim=1. **Split** into `attn_prefill_tt` (S>1, paged_fill_cache + non-paged SDPA) and `attn_decode_step_tt` (S=1, paged_update_cache + paged SDPA decode) — trace-cleaner for v0.4. **Step-by-step**: 1/3 ✅ (`f4743c6` State slots), 2/3 ✅ (`840d584` setup_paged_decode_state, 12/12 gates green), 3a ✅ (`b7af8cf` attn_prefill_tt writes cache, argmax=6993 PASS), 3b PENDING (attn_decode_step_tt + (reset → prefill → decode_step) smoke matches HF gen[6]=1063). | 8-token decode matches v0.3.1.a 7/8; per-step time ~constant | step 3a ✅; 3b next |
