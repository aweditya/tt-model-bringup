...
| **v0.3.1.a** | **Quadratic-prefix multi-step decode smoke** — re-runs full forward on growing prefix each step (O(n²)). No KV cache or ssm_state plumbing needed; validates multi-step CORRECTNESS first. Forks v0.3.0 forward. Iterates via the nm3 harness. HF oracle re-generated with `--gen 8`. **Note: HF generates `Paris, Paris, Paris, ...` loop after the prompt (typical greedy small-model behavior)** — if TT reproduces the same loop, it's strong evidence the math is right. Found a smoke bug: `prompt_ids.npy` is overwritten with `full_ids` after `--gen N`; smoke reads `meta.json["prompt_ids"]` instead. | 8/8 TT tokens match HF | in_progress |
