...
| **v0.1.4** | **MoE Expert-Parallel refactor (True EP, DeepSeek-V3-style)**. 128 experts sharded as 32/chip via `ShardTensorToMesh(dim=0)`. Forward = pre-norm → router → `all_to_all_dispatch` → 32 local experts → `all_to_all_combine` → shared expert (replicated) → residual. SCAFFOLDING LANDED (`upload_moe_layer_ep` + `moe_block_eager_ep`); bootstrap 9.1s (vs full 13.9s); dispatch op call succeeds emitting `[1, 4, 5, 2688]` + `[1, 4, 5, 6]`. KNOWN-OPEN: dispatch output dim 1 = NCHIPS=4 on our (1,4) mesh (cluster_axis=1) — DeepSeek's `tt/moe.py:462` reshapes to `[1, 1, B*S, H]` which requires dim 1 = 1. Need to read tt-metal ttnn unit tests for dispatch output-shape contract before iterating. ~6 expert FFNs/token target vs 128 in Pattern A (21× less compute). Memory ≤ 7.8 GB/chip → unblocks v0.2+. | v0.1.3.b smoke re-runs with EP forward; 3 gates (S/M/B) all cos ≥ 0.999 | 🟡 SCAFFOLDING LANDED (`c5401a9`); forward dim-collapse fix pending |
