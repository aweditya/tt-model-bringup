...
| **v0.3** | Multi-step eager decode: KV cache for 6 attn layers + Mamba2 state (conv + fp32 ssm) for 23 mamba layers. Long-context smoke at L=8192. **Built on v0.2.5 on-device flow** (decoders need state carrying across steps — host round trips per layer are a non-starter). | TT tokens 0..7 match HF token-for-token; needle-haystack at L=8192 ≥75% | pending (blocked by v0.2.5) |
