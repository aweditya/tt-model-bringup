...
| **v0.3.1.c step 3c** | **Correctness debug of decode_step**. Hypotheses to isolate (in order): (1) paged_fill_cache→paged_update_cache hybrid cache state — prefill writes via fill_cache, decode writes via update_cache; do they share consistent slot layout? (2) Mamba2 ssm_state isn't being carried correctly between prefill and decode — check via probe that compares state.ssm_state_np[L] post-prefill against an oracle from numpy SSD. (3) GQA Q split semantics — does each chip's Q[0:16] really map to KV head 0 globally? Most likely IS issue 1 or 3. Isolation probe: 1-layer attn-only smoke that feeds known-good Q + a populated cache, compares paged_sdpa_decode output to a numpy attention reference. | decode_step argmax matches HF gen[6]=1063 (or within 7/8 of v0.3.1.a) | pending |
