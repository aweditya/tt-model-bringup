...
| **v0.4.0h.b.router** | **MoE router on-device** — replace scores readback + host argpartition + topk_indices upload with `ttnn.topk + ttnn.embedding`-gather. Probe at `nemotron3_v040hb_ondevice_router_probe.py` shows cos=0.9997 + 6/8 expert sets match (tie-break drift). **Validation gate**: same 4 needle prompts must generate text that matches the v0.4.1.f/.g baseline. If text diverges → router drift is real, revert. If text matches → safe to ship for trace. | text-equivalent to baseline + n-step chain ≥ 6/7 | next |
