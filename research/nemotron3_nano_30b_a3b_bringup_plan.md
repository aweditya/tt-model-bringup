...
| **v0.4.0h.b** | **On-device router + h reshard** — eliminate scores readback (ttnn.topk + on-device gather via ttnn.embedding) and the h_input → numpy → sharded re-upload (needs replicate→shard primitive). These are the last numpy roundtrips and are trace blockers. | n-step chain ≥ 6/7 + zero `_rb` / `ttnn.to_torch` in moe_block_eager_ep_tt decode path | pending |
| **v0.4.1** | First trace capture of single decode step. With v0.4.0g + v0.4.0h done, the per-step path is pure ttnn. Trace should compress dispatch by ~2-3× on top of eager. | 100 traced steps == 100 eager token-for-token | pending |
