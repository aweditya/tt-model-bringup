...
| **v0.1.4.G0** | **All-to-all spike** — needed because 23 MoE × 1.3 GB > 8 GB/chip. Two paths exist (Pattern A vs True EP); G0 decides. Research round (2 parallel audits, 2026-06-05) found Tenstorrent ships first-class EP primitives (`ttnn.all_to_all_dispatch`, `ttnn.all_to_all_combine`, `ttnn.moe_routing_remap`, `ttnn.moe_expert_token_remap`) + an in-tree DeepSeek-V3 demo at `models/demos/deepseek_v3/tt/moe.py` (sigmoid + bias + group topk — exact arch match). BH status of `all_to_all_dispatch` was broken at #27859, fixed PR #39380 (Mar 2026); UNVALIDATED on our (1,4) P150 mesh. Spike: dispatch → identity → combine round-trip on toy data. | round-trip identity preserved; ttnn ops do not crash on the mesh | pending (task #214) |
| **v0.1.4** | **MoE Expert-Parallel refactor**: 128 experts → 32/chip, fits ~7.8 GB/chip budget. Path chosen by G0: (A) True EP — 6 expert FFNs/token, 21× less compute, blessed by DeepSeek-V3 in-tree demo; (B) Pattern A — fork from 35B `server_35b_ttnn.py:1254-1367` (each chip runs all 32 local experts, mask unselected, all_reduce), known-good but ~95% wasted expert compute. Pre-norm/router/shared/residual stay; only routed-experts dispatch changes. | v0.1.3.b smoke re-runs with refactored forward; 3 gates (S/M/B) all cos ≥ 0.999; memory ≤ 8 GB/chip | pending (task #215, blocked by #214) |
