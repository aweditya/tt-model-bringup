...
| **v0.4.0d** | **Speed up the conv1d kernel itself OR find a path that avoids it for decode**. Hypotheses to test: ① call ttnn.conv1d with smaller input_length (= S only, with zero pad?) ② use ttnn.matmul on the depthwise kernel for S=1 (kernel size 4 × hidden 6144 is a small matmul) ③ check if ttnn ships a depthwise conv1d step-mode variant. Isolation probe: time ttnn.conv1d vs ttnn.matmul on equivalent shapes. | per-step time ≤ 5s eager | pending |
