...
| **v0.4.0c** | **PIVOT: eliminate the conv1d host roundtrip**. The current decode-path conv1d:<br>① `_rb(xBC_tt)` numpy readback<br>② `np.concatenate(conv_state, xBC_np)`<br>③ `ttnn.from_torch(combined).reshape(...)` re-upload<br>④ `ttnn.conv1d(input_length=4, padding=0)`<br>⑤ `ttnn.to_torch(conv_full_tt)` full readback<br>⑥ `ttnn.from_torch(conv_causal_np)` re-upload<br>Replace ①-③ + ⑤-⑥ with on-device prepend (e.g., `ttnn.concat`) and skip the readback/reupload. Conv1d kernel itself stays as-is. | mamba2 layer time drops from 656ms to <100ms; v0.3.3 chain regression matches 6/7 baseline | pending |
| **v0.4.0d** | After v0.4.0c lands, re-profile to see what's next. Likely candidates: SSD wrapper internal padding, conv1d kernel itself, or remaining MoE host topk. | per-step time ≤ 2s eager | pending |
