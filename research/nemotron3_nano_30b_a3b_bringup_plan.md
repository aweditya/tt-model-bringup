...
| **v0.3.1.b** | Proper KV cache + ssm_state on device. Mamba2 `ssm_state_tt[L]` carried; Attention `kv_cache_tt[L]` written at cur_pos via paged_update_cache + read via paged SDPA decode kernel. Drops O(n²) cost from v0.3.1.a. Each step is ~constant time. Required for v0.3.3 long-context. | 8-token decode matches v0.3.1.a; per-step time stable | pending |
