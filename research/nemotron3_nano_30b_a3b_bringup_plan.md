...
| **v0.3.3** | **N-step decode chain via state carry** — prefill 5 tokens once, then loop N=8 decode steps with cur_pos advancing, ssm_state + conv_state + KV cache all evolving. Compare TT chain to HF gen[5..12] token-for-token. Expected: ≥ 7/8 PASS matching v0.3.1.a's bf16 drift floor (single argmax flip with recovery). Acceptance gate before v0.4 trace: state machinery proven across multiple sequential decode calls. | ≥ 7/8 token-for-token match + no crashes/TT_FATAL across the 8 steps | in_progress |
