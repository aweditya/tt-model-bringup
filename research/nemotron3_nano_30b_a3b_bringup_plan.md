...
| **v0.1.4** | **MoE Expert-Parallel refactor (True EP, DeepSeek-V3-style)**. 128 experts sharded as 32/chip via `ShardTensorToMesh(dim=0)`. Forward = pre-norm → router → `all_to_all_dispatch` → 32 local experts → `all_to_all_combine` → shared expert (replicated) → residual. SCAFFOLDING LANDED (`upload_moe_layer_ep` + `moe_block_eager_ep`); bootstrap 9.1s (vs full 13.9s); dispatch op call succeeds emitting `[1, 4, 5, 2688]` + `[1, 4, 5, 6]`. **Shape contract RESOLVED 2026-06-05** ([[reference-all-to-all-dispatch-shape-contract]]): default `output_concat_dim=1` makes dim 1 = source-chip-index×B; with replicated input the 4 slots are identical. **Fix: pass `output_concat_dim=2` + shard seq dim across cluster_axis=1 (pad S=5→8, 2/chip via `ShardTensor2dMesh(dims=(None, 2))`)** — per-chip output becomes `[1, 1, NCHIPS*S_per_chip=8, HIDDEN]`, direct DS-V3 downstream contract, no post-op reshape. In-tree precedent: `tests/ttnn/unit_tests/operations/ccl/test_all_to_all_dispatch.py:1189-1286` (b1s8 case). Stopgap: keep replicated + `[1,4,5,H]→[1,1,20,H]` reshape (4× wasted compute, only if dispatch input contract can't change). ~6 expert FFNs/token target vs 128 in Pattern A (21× less compute). Memory ≤ 7.8 GB/chip → unblocks v0.2+. | v0.1.3.b smoke re-runs with EP forward; 3 gates (S/M/B) all cos ≥ 0.999 | 🟡 SCAFFOLDING LANDED (`c5401a9`); contract audit DONE 2026-06-05; implementation pending |
