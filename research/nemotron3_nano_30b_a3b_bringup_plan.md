...
| **v0.3.1** | Multi-step eager decode: per-layer Mamba2 `ssm_state` and Attention KV cache carried across decode steps (both via ttnn.Tensor — no host bridges for state). 8-token chain match HF token-for-token. Long-context (L=8192) deferred to v0.4 trace (eager too slow but feasible). | TT tokens 0..7 match HF token-for-token | pending (blocked by v0.3.2) |
