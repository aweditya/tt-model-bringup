...
| **v0.4.1** | First trace capture of single decode step. Now that the conv1d kernel is out of the way, the trace win should compress the remaining per-step dispatch (52 layers × N ttnn ops each). | 100 traced steps == 100 eager token-for-token | pending |
