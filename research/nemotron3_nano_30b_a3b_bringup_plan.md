...
| **v0.4.1** | First trace capture — **BLOCKED on #2 indefinitely** per v0.4.0h.c finding. Eager forward stays at 0.26s warm step (~3.8 tok/s steady, 5.0 peak). Next gates: v0.5 single-stream perf via vocab-shard lm_head + RMSNorm fusion + HiFi2; v1 CB. Trace will land when tt-metal upstream ships a deterministic on-device topk variant. | (deferred) | blocked on upstream |
