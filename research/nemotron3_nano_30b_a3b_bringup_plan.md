...
| **v0.4.1.f** | **Nemotron-3 needle-haystack baseline.** Long-context retrieval gate post-v0.4.1.e. Establishes the bar against which the parked on-device router's tie-break drift gets measured. Per Gemma 4 Round 9 lesson: needle failures can be prompt-shape artifacts (IT model echoing its instruction), not precision floors. Probe at L=128/512, 2 trials each. Time-budget ~15 min. | retrieval rate across (L, frac) | in progress (running 2026-06-05) |
| **v0.4.1** | First trace capture of single decode step. Blockers cleared: #1 (embed), #3 (replicate→shard via v0.4.1.e), #4 (final_norm/lm_head/argmax). Remaining: #2 (MoE router host topk) — needs the long-context baseline (v0.4.1.f) to decide whether ttnn.topk tie-break drift is shippable. | 100 traced steps == 100 eager token-for-token | blocked on v0.4.1.f outcome |
