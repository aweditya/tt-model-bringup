...
| **v0.3.1.c step 3c** | **Correctness debug of decode_step via teacher-forced per-layer ladder**. HF oracle has `hidden_states.npy` shape `(53, 13, 2688)` = per-layer per-position ground truth. For each layer L at decode_pos=5: feed HF hidden_states[L][5] (teacher-forced clean input), run TT layer L decode, compare to HF hidden_states[L+1][5]. First divergence layer = bug locus. REUSE: forks `cb35_per_layer_drift_pos1.py` + `gm4_per_layer_drift_pos1.py` patterns. Probe: `nemotron3_v031c_decode_ladder_smoke.py`. Hypotheses to test once first-fail identified: (a) paged_fill_cache→paged_update_cache hybrid layout, (b) GQA Q split semantics (Q[0:16]→KV head 0 vs interleaved), (c) Mamba2 ssm_state carry between prefill and decode_step. | first-fail layer identified; cos ≥ 0.99 per layer once fixed | in_progress |
