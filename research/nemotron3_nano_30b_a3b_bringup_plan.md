...
| **v0.1.2.b** | **L0 — conv1d (depth-wise K=4, pad=3) + silu**. Two options: (a) ~10 ttnn ops (slice+mul+add) on padded [1,11,6144] across 4 kernel positions, OR (b) `ttnn.conv1d` if it supports `groups=conv_dim`. HF hook captures the FULL conv output [1, 6144, 8] (symmetric pad, pre-causal-slice). | conv1d output cos ≥ 0.999 vs HF L0_conv1d | pending |
| **v0.1.2.c** | **L0 — full forward**. Split into z/x_BC/dt → conv1d_step → silu → split into x/B/C → SSD recursive loop (5 iters of `mamba2_decode_step_ttnn` wrapper, accumulating ssm_state) → MambaRMSNormGated (head_dim=64 groups, gated by z·silu) → out_proj → residual. | L0_norm + L0_out_proj + L0_mamba2_mixer_out all cos ≥ 0.999; block_out vs hidden_states[1] cos ≥ 0.999 | pending |
