...
| **v0.3.3** | Long-context validation — mirrors 27B/35B's `[[needle-haystack-qb1]]` + `[[bf16-prefill-drift-cliff]]` pattern. Three sub-probes + a dev harness. | (a) per-pos cos at L=200+; (b) sliding pos 1025 with-perturbation cos ≥ 0.999 vs without; (c) needle retrieved verbatim. | **PARTIAL DONE** — (a) DONE commit `a7eef0d` (L=215; argmax 95.81%, median cos 0.9932, 5th-pct 0.9779 — all 3 bf16-aware gates PASS). (c) probe shipped 2026-06-03 — `gm4_v033c_needle_haystack.py`, default L=100/256/512 frac=0.5, gates L=100 ALL Y / L=256 ≥75% / L=512 ≥50% Y+P; first run in flight via harness. Dev harness LIVE commit `<gm4-harness>` — `gm4_dev_harness.py` saves ~80s per iter, `bash scripts/run_harness_tmux.sh gm4` to launch. (b) sliding-window invariance still pending — needs > 1024-tok HF oracle which is expensive on CPU. |
