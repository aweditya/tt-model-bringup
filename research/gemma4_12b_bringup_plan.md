...
| v0.3 | KV cache + paged SDPA with `sliding_window_size=1024` | 8-tok generation matches HF token-for-token | IN FLIGHT — **setup DONE 2026-06-03 commit `cb4e299`**: KV caches per layer (sliding sharded, global replicated), SDPA program/memory/compute configs, RoPE tables (sliding + p-RoPE global). v0.2 probe re-run with these in place: STILL PASSES. Only forward changes remain. See sub-staging below. |
| v0.4 | Trace capture: `update_input_buffers` (host→device for tok_buf/cur_pos_buf/rot_idxs_buf) + `forward_token_gm4_inner` (reads ONLY those buffers, returns on-device argmax) + `ensure_decode_trace` (2-eager-warmup + capture per [[ttnn-multi-trace-two-phase-warmup]]) + `step_forward_traced` (update + execute_trace + readback). Mirrors 27B prod `server_tp.py:1587-1694, 2142-2186`. | 100 traced steps == 100 eager steps token-for-token. | **DONE 2026-06-03 commit `626c67a`** — 100/100 match on a non-trivial teacher-forced + free-run sequence (first 10 tokens `[258882, 236743, 529, 506, 236764, 496, 3207, 529, 1610, 236764]`). Eager 182.7 ms/tok → traced **51.3 ms/tok = 3.56× speedup** out of the box; v1 CB will amortise the per-step readback further. Trace capture cost: 693 ms one-time. trace_region_size=400 MB suffices for the 48-layer decode trace. Iterates via dev harness. |
| **v1 CB** | Continuous batching at B>1. Forks `server_tp_cb.py` → `server_gemma4_unified_cb.py`. `setup_cb_state`, `cb_reset_states`, `update_input_buffers_batched`, `_apply_full_rope_b`, `_shard_for_paged_write_b`, `_layer_sliding_batched` (2 SDPA per layer, NKV=1), `_layer_global_batched` (canonical (8,4) cfg, NKV=1 replicated), `_layer_forward_batched`, `forward_batch_gm4_inner`, `step_forward_cb`. | 3a B=1 batched == single-slot v0.4; 3b identical-slot; 3c distinct-slot; PASS at B=4. | **DONE 2026-06-04 commit `<v1-final>`** — all 4 gates PASS at B=1, B=2, and B=4. v1.0 alloc 4.0s/9.7s. v1.4 3a: B=1 == single-slot bit-identical. v1.5 3b: identical-slot bit-identical at B=2. v1.5 3c: distinct prompts produce distinct sequences, each matching its B=1 reference (no cross-talk). **v1.6 B=4 final gate**: 4 slots (2 identical + 2 distinct), all match their B=1 refs, 3b and 3c both PASS. B=4 forward 6.2s for 6 steps = ~1.0s/step eager (1.6s/tok aggregate at B=4 = 2.5 tok/s/slot ≈ 10 tok/s aggregate; trace will amortise per [[ttnn-multi-trace-two-phase-warmup]]). Harness reload now also reloads `server_gemma4_*` siblings ([[feedback-harness-state-version-skew]]). Ready for v2 HTTP wire-up. |
...
| v0.3.0.1 | Split sliding SDPA into 2 calls per layer (Q halves × per-KV-head caches, NKV_PER_CHIP=1 each — matches 35B contract cleanly). 2 caches per sliding layer. | cos ≥ 0.999 on mixer_out + final_norm + argmax matches HF | **DONE 2026-06-03 commit `e2ae9f2`** — final_norm cos 0.999601 / logits cos 0.999370 / argmax MATCHES HF. Same total memory as 1-cache variant. Slightly BETTER cos than v0.2's no-cache shortcut (bf16 noise direction). |
| v0.3.1 | Multi-step decode loop + RoPE at pos > 0. | TT tokens 0..5 match HF teacher-forced argmax_per_position[0..5] | **DONE 2026-06-03 commit `c97bf15`** — root cause: SDPA `scale=1.0/sqrt(head_dim)` (wrong); Gemma 4 text attention sets `self.scaling = 1.0` (HF `modeling_gemma4.py:1178`), confirmed in Tenstorrent's in-tree demo (`decode.py:144`, `operations.py:15`). Masked at pos 0 (single-token softmax = 1.0 regardless). After-fix multi-step: pos 0..5 cos_final all ≥ 0.997 (was 0.26 at pos 1); per-layer ladder L0-L46 cos > 0.996 at BOTH pos 0 and pos 1. 5/6 argmax PASS — pos 4 cos=0.9984 but argmax differs (bf16 chain noise on near-tie per [[bf16-chain-drift-at-B-gt-1]]; cosine is the production gate). Memory: [[feedback-gemma4-sdpa-scale-1]]. Bisection probes ship: `gm4_sliding_write_read.py`, `gm4_global_write_read.py`, `gm4_rope_lookup.py`, `gm4_per_layer_drift_pos1.py`. Debug knobs: GM4_DEBUG_POS, GM4_ROPE_ZERO, GM4_SKIP_SLIDING, GM4_SKIP_GLOBAL. |
| v0.3.2 | Free-run greedy 16 tokens after teacher-forced prefill. | Coherent English continuation (no hard token-for-token gate — bf16 will diverge from HF). | **DONE 2026-06-03 commit `acb20a6`** — "The capital of France is a city of art, culture, and history." (16 tokens grammatically + semantically correct, then loops on `<image|>` because there's no EOS stop in the raw forward — fine; CB/HTTP layer will handle it). Probe: `gm4_v032_freerun.py`. |
| **v0.3.3** | Long-context validation — mirrors 27B/35B's `[[needle-haystack-qb1]]` + `[[bf16-prefill-drift-cliff]]` pattern. Three sub-probes + a dev harness. | (a) per-pos cos at L=200+; (b) sliding pos 1025 with-perturbation cos ≥ 0.999 vs without; (c) needle retrieved verbatim. | **(a) + (c) DONE 2026-06-03** — (a) commit `a7eef0d` L=215; argmax 95.81%, median cos 0.9932, 5th-pct 0.9779 (all 3 bf16-aware gates PASS). (c) commit `f374407`/run via harness — 3/3 Y at L=100/256/512 frac=0.5 (passwords retrieved verbatim, e.g. L=512 `FWD7SWFY` → `**FWD7SWFY**`). Dev harness LIVE — `gm4_dev_harness.py` saves ~80s per iter, `bash scripts/run_harness_tmux.sh gm4`. (b) sliding-window invariance still pending — does not require an HF oracle (just two TT forwards perturbing token 0), but each forward is > 1024 tokens × ~175 ms/tok = ~3 min, so two forwards ~6 min. Will land when v0.4 trace ships and drops per-tok latency. |
