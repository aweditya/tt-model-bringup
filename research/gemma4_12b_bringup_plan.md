...
| **v1 CB** | Continuous batching at B>1. Forks `server_tp_cb.py` → `server_gemma4_unified_cb.py`. `setup_cb_state`, `cb_reset_states`, `update_input_buffers_batched`, `_apply_full_rope_b`, `_shard_for_paged_write_b`, `_layer_sliding_batched` (2 SDPA per layer, NKV=1), `_layer_global_batched` (canonical (8,4) cfg, NKV=1 replicated), `_layer_forward_batched`, `forward_batch_gm4_inner`, `step_forward_cb`. | 3a B=1 batched == single-slot v0.4; 3b identical-slot at B=2; 3c distinct-slot at B=2; PASS at B=4. | **v1.0 + v1.4 3a + v1.5 3b + v1.5 3c ALL PASS at B=1/B=2 2026-06-04**. v1.0 alloc smoke 4.0s/9.7s at B=2/B=4. **v1.4 3a**: B=1 CB == single-slot: both `[258882, 236743, 529, 506, 236764, 496]`. **v1.5 3b**: identical-slot bit-identical. **v1.5 3c**: distinct slots, each matches its own B=1 reference (no cross-talk). Next: v1.6 B=4 same gates + trace capture at B=4. Dev-harness reload pattern needed an update to reload `server_gemma4_*` sibling modules so sys.modules picks up CB edits ([[feedback-harness-state-version-skew]]). |
