...
| v0.3.1 | Multi-step decode loop: pre-allocated `tok_buf` written out-of-trace; update_input_buffers advances tok+cur_pos+rot_idxs per step. Maintain KV cache across steps. RoPE at pos > 0 (non-trivial). | TT tokens 0..7 match HF teacher-forced argmax_per_position[0..7] | **IN FLIGHT, STUCK at 3/6 PASS** — pos 0 cos_final=0.9995 (bit-clean); **pos 1 cos_final=0.2618 (cliff)**; pos 2-5 cos_final 0.42-0.62 (medium drift, argmax matches HF on 2 of 4 by luck). Ruled out (all PASS with isolation probes): paged write/read at sliding shape (gm4_sliding_write_read.py 6/6 cos 0.9996+), paged write/read at global shape (gm4_global_write_read.py 6/6 cos 0.9996+), ttnn.embedding row lookup (gm4_rope_lookup.py), on-device cur_pos_buf / rot_idxs_buf values (GM4_DEBUG_POS verified [1,1,1,1] across mesh), RoPE math (GM4_ROPE_ZERO=1 gives same 3/6 pattern). Code hardening landed: canonical Tenstorrent in-tree Gemma 4 SDPA config for global (commit `5c96abb`), view-decay cleanup in `_apply_full_rope` and sliding loop, in-place buffer update via `copy_host_to_device_tensor` (27B prod pattern). **Open**: pos 1 specifically (not all pos > 0) hits a sharp cliff; GM4_SKIP_GLOBAL / GM4_SKIP_SLIDING don't cleanly isolate. Next session: capture per-layer h cosines through 48 layers at pos 1 to find where drift starts. |
