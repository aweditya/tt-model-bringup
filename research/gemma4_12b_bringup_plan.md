...
| **v0.3.3** | Long-context validation — mirrors 27B/35B's `[[needle-haystack-qb1]]` + `[[bf16-prefill-drift-cliff]]` pattern. Three sub-probes: (a) **per-pos cosine ladder at L=128** vs a longer HF oracle — gates the basic forward at moderate context; (b) **sliding-window correctness at L=1024+1**: at pos > 1024 a sliding layer's output MUST be invariant to changes in tokens before pos-1024 (verified by perturbing token 0 vs token 1024 at pos 1025); (c) **needle-haystack retrieval at L=100, L=500, L=1024** — fork `experiments/utils/needle_haystack_35b_ttnn.py` for the Gemma 4 tokenizer + prompt structure. | (a) all-pos cos_final ≥ 0.99 vs HF at L=128; (b) sliding pos 1025 with-perturbation cos ≥ 0.999 vs without; (c) needle retrieved verbatim at L ≤ 500, retrieval ≥ 80% at L=1024. | NOT STARTED — fork map: (a) extend `experiments/utils/hf_reference_gemma4_12b.py` with `--prompt-len 128` flag (longer prompt), then fork `gm4_v031_multistep_cos.py` → `gm4_v033a_long_cos.py`; (b) fork `gm4_v031_multistep_cos.py` → `gm4_v033b_sliding_window_invariance.py` (two parallel forwards, perturb tok 0 in one, compare pos 1025 cos); (c) fork `experiments/utils/needle_haystack_35b_ttnn.py` → `needle_haystack_gemma4_12b.py` + `_hf.py`. Reuse mandate: cite each fork base in commit message. **Likely gotchas to watch**: MAX_KV=4096 cap (bump if needed); sliding cache wrap if pos > MAX_KV; global p-RoPE behavior past sliding-window boundary; bf16 drift compounding past 256-512 tokens ([[bf16-prefill-drift-cliff]]). |
