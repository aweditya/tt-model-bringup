...
| v0.4 | Trace capture: `update_input_buffers` (host→device for tok_buf/cur_pos_buf/rot_idxs_buf) + `forward_token_gm4_inner` (reads ONLY those buffers, returns on-device argmax) + `ensure_decode_trace` (2-eager-warmup + capture per [[ttnn-multi-trace-two-phase-warmup]]) + `step_forward_traced` (update + execute_trace + readback). Mirrors 27B prod `server_tp.py:1587-1694, 2142-2186`. | 100 traced steps == 100 eager steps token-for-token. | **IN FLIGHT 2026-06-03** — server functions shipped; bootstrap bumped to `trace_region_size=400_000_000` (50 MB default OOMs the 48-layer decode trace). Validator: `gm4_v04_trace_validate.py`. Iteration via dev harness. |
