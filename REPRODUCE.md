...
A fresh-clone recipe for [Tenstorrent QuietBox][quietbox] (4× Blackhole P150).
Confirms the production chat server, the legacy single-chip demos, and the
owned-kernel validation gates.
...
- [Setup](#setup)
- [Reproduce — chat server (Qwen3.6-27B CB)](#reproduce--chat-server-qwen36-27b-cb)
- [Reproduce — legacy single-chip demos](#reproduce--legacy-single-chip-demos)
- [Reproduce — owned_ops kernel gates](#reproduce--owned_ops-kernel-gates)
- [Device configuration](#device-configuration)
- [Verifying results](#verifying-results)
- [Troubleshooting](#troubleshooting)
- [Archived path: pre-CB single-stream servers](#archived-path-pre-cb-single-stream-servers)

---

## Hardware

- **Tenstorrent QuietBox** (4× Blackhole P150) for the production 27B / 35B /
  Gemma 4 paths — they need a `(1, 4)` mesh with working `FABRIC_1D`.
- **Single P150** is enough for the 8B-class demos under [`models/`](models/),
  with ≥ 64 GB system RAM for HuggingFace shard loading.
...
Configure HuggingFace access: `uv run hf auth login`, or copy
[`.env.example`](.env.example) → `.env` and set `HF_TOKEN=hf_…`.

Device-access smoke (useful when debugging the TT-NN install):
...
## Reproduce — chat server (Qwen3.6-27B CB)

The canonical path. The continuous-batching engine, OpenAI-compatible HTTP,
and the production 4× P150 graph:
...
Once `/health` returns 200, hit the endpoint:
...
> The README §"Chat server" section documents streaming, sampling, `/metrics`,
> and the load-test (`experiments/cb/load/concurrent_chat.py`).

**Observed SLO** (8 clients × 60 s, 2026-05-30): 0 errors / 36 requests /
15 tok/s aggregate / TTFT p99 = 176 ms.

---

## Reproduce — legacy single-chip demos

The prod CB server holds device 0 — stop it first.
...
| Demo | What it tests | Expected | Measured |
|---|---|---|---|
...
Fresh-clone validation (2026-05-22, second QuietBox): all six PASS within
2–7 % of baseline.
...
`test_*.py`. The two production ops (BF16 ladder vs CPU oracle):
...
See [`experiments/owned_ops/README.md`](experiments/owned_ops/README.md) for
the full op index + install path.

---

## Device configuration

All single-P150 experiments use device 0 only. Compute kernel config:
...
| Out of memory | 8B models need ~25 GB device DRAM; no other processes on the device. |
| Firmware mismatch | `tt-smi -r 0` or follow TT-Metal docs. |
| Slow model download | First run downloads weights from HuggingFace (8B ~15 GB across 4 shards; 27B cold ~6 min). |
| Server hangs / fabric wedged after a hard-kill | `tt-smi -r 0,1,2,3` then restart. Always prefer the script's `stop` over `kill -9`. |

---

## Archived path: pre-CB single-stream servers

The pre-CB single-stream Unix-socket servers (`server.py`, `server_tp.py`
wrapper, `server_35b.py`) and their launch scripts were moved to
[`archive/pre_cb_server_stack_2026-06-04/`](archive/pre_cb_server_stack_2026-06-04/)
on 2026-06-04, superseded by the continuous-batching HTTP server above.
Steady-state numbers at retirement: single-seq TP 12.93 tok/s, single-chip
5.14 tok/s.
